...
- For Blender tool choice, runtime API lookup or live pose/viewport inspection, use [BLENDER_MCP_COMPARISON.md](../../../docs/BLENDER_MCP_COMPARISON.md). The tested separate `blender_lab` bridge uses portable5.2.1 and loopback19877; the existing4.5/`blender` workflow remains available. Prefer bounded source-copy CPU operations; a tool installation is not art or animation admission.
...
- For current production, use [PRODUCTION_LOOP_POLICY.md](../../../docs/PRODUCTION_LOOP_POLICY.md): one playable outcome, early integration, independent review, and up to three substantial passes (initial implementation plus two focused repairs). Stop early on PASS; repeated gaps require a changed method. The two-batch trial in [ALPHA_WORKFLOW.md](../../../docs/ALPHA_WORKFLOW.md) remains historical evidence. Preserve all save/physics/determinism/offline closure requirements and the separate three-batch overnight limit.
...
- If Chris explicitly accepts a displayed candidate below the numerical art threshold, record the exact version/hashes and his decision, then integrate that version. Preserve the independent score as historical target comparison; do not inflate it or extend acceptance to another version/family. Physical access, collision, export and performance still need proof.
...
- Before replacing region scenery, resolve every old prop's asset gameplay role. `props` contains harvestables and live Wildkin as well as scenery: preserving only `resources` and `creatures` silently deletes ore, food and encounters. Compare all prior gameplay IDs/roles against the composed world, then check required renewable materials and physical approaches. A new visually similar prop is not the same saved identity.
