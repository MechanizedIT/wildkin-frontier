...
const manifest={createdAt:new Date().toISOString(),source:'Current authored dev world with native Three.js scene/terrain/visual constructors; no player save loaded',sourceWorldPath,sourceWorldSHA256:crypto.createHash('sha256').update(await fs.readFile(sourceWorldPath)).digest('hex'),reproduce:`node tools/review/capture-visual-atlas.mjs --url ${base} --out ${out}`,maps:[],models:[],contactSheets:[],errors:[]};
...
try{
 await page.goto(new URL('tools/review/visual-atlas.html',base).href);await page.waitForFunction(()=>window.atlas?.ready||window.atlas?.error,{timeout:60000});const failure=await page.evaluate(()=>atlas.error);if(failure)throw Error(failure);await page.waitForLoadState('networkidle');
 const regions=await page.evaluate(()=>atlas.regions);
 for(const r of regions)for(const expanded of r.id==='camp'?[false,true]:[false]){
  ...
  await page.locator('canvas').first().screenshot({path:record.path});await page.evaluate(()=>atlas.planning());await page.screenshot({path:record.planningPath});manifest.maps.push(record);await save();console.log(`MAP ${id}: ${record.path}`);
 }
 ...
 const records=[model('explorer','Explorer',['explorer-v2']),model('mossling','Mossling',['mossling-v3']),model('sapwood','Sapwood harvest tree',['sapwood-v1']),
  ...
  model('rootfall-candidate','Rootfall — pending candidate',['left','right','center'].map(n=>({path:`art/source/rootfall-v1/candidate-v1/final/rootfall-${n}.glb`})).concat([-1.65,1.65].map(x=>({path:'art/source/rootfall-v1/candidate-v1/final/rootfall-seam.glb',offset:[x,0,1]}))),'assembly','PENDING · not integrated / not admitted'),
 ...
 for(const r of records){const metrics=await page.evaluate(r=>atlas.model(r),r);r.path=`${out}/models/${r.id}.png`;Object.assign(r,metrics);await page.locator('canvas').first().screenshot({path:r.path});manifest.models.push(r);await save();console.log(`MODEL ${r.id}`);}
 ...
 for(const file of [...manifest.maps.flatMap(r=>[r.path,r.planningPath]),...manifest.models.map(r=>r.path),...manifest.contactSheets]){const bytes=await fs.readFile(file);manifest.files??={};manifest.files[path.relative(out,file).replaceAll('\\','/')]={bytes:bytes.length,sha256:crypto.createHash('sha256').update(bytes).digest('hex')};}
