...
The production path normalizes the fitted FBX rig and mesh together to meters.
Horizontal Hips travel is held for locomotion while vertical pelvis bob stays
intact; physics-owned actions hold all Hips translation. Rotations remain on
the compatible source skeleton. The experimental retarget below uses separate
rest-axis conversion and is retained only as a rejected diagnostic approach.
...
def auto_skin_candidate(options, output):
    ...
    report = {
        ...
        "runtime_note": "FBX/glTF bone mixamorig:RightHand loads as mixamorigRightHand in the project's Three.js loader; use the actual loaded name for the fitted hand anchor. Each direct Mixamo action remains subject to state-timing and game-motion review. Legacy actions, when present, are separately experimental.",
    }
