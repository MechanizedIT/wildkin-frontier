...
def code_identity_selector(target_code, local_trace):
 def global_trace(frame,event_name,arg):
  return local_trace if event_name=='call' and frame.f_code is target_code else None
 return global_trace
def native_phase_trace(torch, output, imported_postprocess):
 source=Path(imported_postprocess.__file__).resolve()
 if source.suffix!='.py' or sha(source)!=POSTPROCESS_SHA: raise RuntimeError('imported postprocess source hash mismatch: '+str(source))
 target_code=imported_postprocess.to_glb.__code__; hooks={114:'cumesh-create',129:'bvh-create',194:'remesh',219:'simplify',258:'uv',297:'rasterize',324:'bake-bvh',419:'return'}
 samples=[]
 def local_trace(frame,event_name,arg):
  ...
  return local_trace
 return code_identity_selector(target_code,local_trace),samples,{'imported_source_filename':str(source),'imported_source_sha256':sha(source),'to_glb_firstlineno':target_code.co_firstlineno}
def postprocess_child(args,plan):
 ...
 trace,samples,trace_binding=native_phase_trace(torch,Path(args.public_output),o_voxel.postprocess)
 try:
  ...
  event(args.public_output,'postprocess:complete',output_sha256=sha(target),bytes=target.stat().st_size,inventory=inventory,trace_binding=trace_binding,gpu_samples=samples+[gpu_sample(torch,'postprocess:complete')]); stop.set()
  if not samples:
   event(args.public_output,'HOLD_PHASE_TRACE',trace_binding=trace_binding,artifact_path=str(target)); raise RuntimeError('HOLD_PHASE_TRACE: GLB retained but no expected native hooks observed')
...
class Tests(unittest.TestCase):
 ...
 def test_trace_selector_uses_code_identity(self):
  import types
  def target(): pass
  def sibling(): pass
  local=lambda frame,event,arg:'local'
  select=code_identity_selector(target.__code__,local)
  self.assertIs(select(types.SimpleNamespace(f_code=target.__code__),'call',None),local)
  self.assertIsNone(select(types.SimpleNamespace(f_code=sibling.__code__),'call',None))
...
def main():
 ...
 if not a.retry_postprocess: a.private_output.mkdir(parents=True)
 a.public_output.mkdir(parents=True); write_json(a.public_output/'run-plan.json',plan); h=mutex()
 try:
