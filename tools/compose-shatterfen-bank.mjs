...
const BANK_PAINT_ROUTES=Object.freeze([
...
// This narrow post-terrain transaction owns only the retained Fen-bank paint
// islands and their feet. It deliberately does not touch landforms, water,
// admitted asset metadata, or the newer observatory route topology.
export function applyFenBankSurfaceAndGrounding(world){
 const region=world?.regions?.find(r=>r.id==='section_2');if(!region?.surface)throw Error('Fen bank requires existing terrain');
 const surface=region.surface,anchorRoute=surface.routes.find(r=>r.id==='fen-observatory-forecourt')??surface.routes.find(r=>r.id==='observatory-island-spur');
 // The established Author sequence applies this bank pass before the uplands
 // replacement. Accept its former route anchor as well as the V2 forecourt;
 // the helper itself still only owns local bank paint and grounding.
 if(!anchorRoute)throw Error('Fen bank requires an observatory forecourt or legacy island spur');
 surface.routes=surface.routes.filter(r=>!r.id.startsWith(PREFIX));
 surface.routes.push(...BANK_PAINT_ROUTES.map(route=>structuredClone(route)));
 for(const part of FEN_BANK_COMPONENTS){
  const bankProp=region.props.find(p=>p.id==='prop_'+PREFIX+part.id),asset=world.visualAssets.find(a=>a.id===bankProp?.visualAssetId);
  if(!bankProp||!asset?.collision)throw Error(`Fen bank requires existing ${part.id} collision support`);
  bankProp.pos.y=supportBottom(surface,part,asset.collision);
 }
 for(const bankProp of region.props.filter(p=>p.id.startsWith('prop_'+PREFIX)&&!p.id.includes('outcrop_'))){
  bankProp.pos.y=Number(getSurfaceHeight(surface,bankProp.pos.x,bankProp.pos.z).toFixed(4));
 }
 return region;
}

function installBankAssets(world){
 for(const [name,s] of Object.entries(MODELS)){
  ...
  if(!world.visualAssets.some(asset=>asset.id===assetId))world.visualAssets.push({id:assetId,displayName:`Fen bank ${name==='left'?'low ledge':'high shoulder'}`,category:'Alien ecology',version:1,parts:[],gameplay:{role:'prop'},collision,model:{path:`assets/models/fen-bank-outcrop-${name}-v1/model.glb`,scale:1,pivot:{x:0,y:0,z:0}}});
 }
 if(!world.visualAssets.some(asset=>asset.id==='asset_'+PREFIX+'frond'))world.visualAssets.push({id:'asset_'+PREFIX+'frond',displayName:'Fen folded bank frond',category:'Alien ecology',version:1,parts:frondParts(),gameplay:{role:'prop'},collision:null});
}

export function composeShatterfenBank(world){
 const region=world.regions.find(r=>r.id==='section_2');if(!region?.surface)throw Error('Fen bank requires existing terrain');
 const surface=region.surface;
 installBankAssets(world);
