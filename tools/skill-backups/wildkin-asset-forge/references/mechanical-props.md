...
- Check reference supports against the intended motion before modeling. The first Fen receiver image combined two fixed side bearings with a third pedestal touching the bowl, so its proposed tilt could not work. The reviewed correction removes that third support and uses one aligned horizontal axis. Model and inspect the actual swept assembly against its fixed supports at both limits; an attractive neutral image or visible air gap does not prove the sweep.
