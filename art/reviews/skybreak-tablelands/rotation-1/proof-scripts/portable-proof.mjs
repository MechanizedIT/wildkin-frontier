...
for(const [label,url,file] of [
 ['portable-route','http://localhost:8081/',`.dream-loop/skybreak-rotation-1/${phase}-save-private.json`],
 ...
 await page.screenshot({path:`art/reviews/skybreak-tablelands/rotation-1/${label}.png`});
 if(label==='portable-route') {await page.setViewportSize({width:915,height:412});await page.waitForTimeout(800);await page.screenshot({path:'art/reviews/skybreak-tablelands/rotation-1/portable-landscape.png'});}
 fs.writeFileSync(`.dream-loop/skybreak-rotation-1/${label}-restored-private.json`,JSON.stringify(restored.payload));
 reports.push(row);console.log(JSON.stringify(row));await context.close();
}
await browser.close();fs.writeFileSync('art/reviews/skybreak-tablelands/rotation-1/portable-proof.json',JSON.stringify(reports,null,2));
