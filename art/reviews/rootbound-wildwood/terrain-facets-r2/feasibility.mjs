...
for (let z = rect.minZ; z <= rect.maxZ; z += 2) for (let x = rect.minX; x <= rect.maxX; x += 2) {
  ...
  const tint = params.facetFan.vertexColorMultiplierDeltaAtFullWeight;
  const signedFacetWeight = term.weight * (term.left - term.right);
  vertices.push({ x, z, ...base, queryMinusMeshM: q.height - base.y, ...term, signedFacetWeight,
    candidateY: base.y + term.deltaM,
    candidateVertexColorMultiplier: tint.map(value => 1 + value * signedFacetWeight) });
}
