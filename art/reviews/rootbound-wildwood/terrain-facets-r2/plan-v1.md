...
- existing terrain main material is already flat shaded (src/world/frontierChunkRuntime.js), so this tests actual mesh planes and vertex color rather than a normal-only substitute.
...
The next private render comparison can apply the computed vertex deltas and candidate RGB values only to cloned current chunk buffers. It must preserve the native 2 m index arrays, flatShading, lighting and camera. It is renderer-only:
...
If that native A/B is visibly useful and a later owner-authorized source pass is selected, the same **single final term** must be placed in Rootbound's authoritative height/color chain. The likely owners to review together are:
...
No normal API is proposed. Correlated color is deliberately small (+[.022,.035,.012] at full raised/recessed weight) and must be evaluated only as a color blend tied to the same local fan—not new texture noise.
