...
const OUT = new URL('./repair-support-clearance.json', import.meta.url);
...
const plannedPlacements = Object.freeze([
...
const placements = Object.freeze(plannedPlacements.map(p => { const actual=ROOTBOUND_CURATED_SCENERY.find(a=>a.key===p.key); if(!actual) throw new Error('Missing '+p.key); return {...p,...actual}; }));
