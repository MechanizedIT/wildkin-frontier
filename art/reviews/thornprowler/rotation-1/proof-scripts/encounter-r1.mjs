...
const out='art/reviews/thornprowler/rotation-1';
...
await page.evaluate(async()=>{
 const g=window.__game,x=-2868,z=-1248;
...
const snap=()=>page.evaluate(()=>{
 ...
 const c=g.creatureSystem.getCreatures().filter(c=>c.state.visualAssetId==='asset_thornprowler').sort((a,b)=>Math.hypot(a.state.pos.x-pos.x,a.state.pos.z-pos.z)-Math.hypot(b.state.pos.x-pos.x,b.state.pos.z-pos.z))[0];
...
const report={note:'Isolated copied earned save, positioned once on supported Fungal Hollow ground near the existing fixed Thornprowler home f1:w:-58:-26:450 (-2868,-1260). No new creature fixture. Ordinary player keys and natural AI thereafter; no AI-state or pose manipulation. Actual baked R1 factory model, existing aggressive rusher behavior.',start:await snap(),trace:[],frames:[],errors};
const seen=new Set();let lunge=false,recovery=false;
try {
 for(let i=0;i<300;i++){
  const s=await snap();report.trace.push(s);
  if(!s.creature)throw Error('Thornprowler not resident');
  if(!seen.has(s.creature.ai)){
   seen.add(s.creature.ai);const file=`r1-encounter-${s.creature.ai.toLowerCase()}.png`;
 ...
 await page.screenshot({path:out+'/r1-encounter-retreat.png'});
}catch(e){report.failure=e.message;}
finally{
 report.seen=[...seen];report.lunge=lunge;report.recovery=recovery;
 fs.writeFileSync(out+'/r1-encounter-proof.json',JSON.stringify(report,null,2));
