...
PLAN=SRC/'construction-data-r2.json'; EXPECTED='0dc4dad6951ca7b3aca36a502b04b50f26d64fb2a014a3b0c3b4e2e5c11b211d'
assert hashlib.sha256(PLAN.read_bytes()).hexdigest()==EXPECTED
data=json.loads(PLAN.read_text(encoding='utf-8-sig'))
assert (SRC/'construction-plan-r2-review.md').exists(), 'Independent review missing'
verts=[(x,-z,y) for x,y,z in data['vertices']];faces=data['faces']
bpy.ops.wm.read_factory_settings(use_empty=True)
me=bpy.data.meshes.new('IronspineBankLiteralR2');me.from_pydata(verts,[],faces);me.update()
changed=me.validate(verbose=True);me.calc_loop_triangles()
beforeNormals=[list(p.normal) for p in me.polygons]
# Audit literal normals first. R2 expects consistent input; do not silently
# repair it. Any failure remains a saved pre-render result for review.
obj=bpy.data.objects.new('GEO-IronspineNarrowBank-R2',me);bpy.context.collection.objects.link(obj)
...
receipt()
assert not changed and len(me.loop_triangles)==230 and components==1 and set(edges.values())=={2} and volume>0 and metrics['zeroAreaTriangles']==0 and error<1e-6 and metrics['directedEdgeConsistent']
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'ironspine-bank-r2.blend'))
