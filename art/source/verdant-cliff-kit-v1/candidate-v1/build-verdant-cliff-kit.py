"""Build the Verdant V1 scenic-cliff family from explicit low-poly rock masses.
...
for y in range(256):
    for x in range(256):
        pixels.extend([srgb(c) for c in PALETTE[(y // 64) * 4 + x // 64]] + [1])
...
def build_buttress():
    ...
    specifications = [
        ...
        ("Buttress right tower", (.58,.12), [(0,1.04,.72,0,0),(.72,.97,.68,.03,.02),(2.08,.78,.57,.08,.00),(3.18,.64,.48,.00,.02),(3.88,.43,.34,-.10,.03)], .42),
        ("Buttress left shelf", (-.73,-.10), [(0,.86,.66,0,0),(.48,.85,.64,-.02,0),(1.35,.75,.58,.06,.03),(2.35,.55,.42,.16,.02),(2.72,.35,.31,.22,.03)], -.16),
        ("Buttress forward stratum", (-.15,-.62), [(0,1.22,.42,0,0),(.32,1.16,.41,.02,0),(1.02,1.03,.37,-.04,.00),(1.43,.66,.30,-.19,.01)], .20),
    ]
...
manifest = {
    "assetFamily": "verdant-cliff-kit-v1 candidate-v1",
