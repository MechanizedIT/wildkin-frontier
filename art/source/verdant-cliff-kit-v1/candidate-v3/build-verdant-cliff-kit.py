...
PALETTE = [
    (105, 105, 103), (115, 113, 108), (124, 120, 113), (132, 127, 119),
    (136, 128, 118), (144, 135, 123), (153, 142, 129), (161, 150, 136),
...
def block(name, corners, bevel, seed, moss=False):
    ...
    for face in bm.faces:
        ...
        if face.calc_area() > .18 and face.normal.z > -.65:
            # 1 centre per major face: wide irregular facets, bounded 2cm lift.
            center = face.calc_center_median()
            anchor = face.verts[rng.randrange(len(face.verts))].co
            center = center.lerp(anchor, rng.uniform(.10, .23))
            center += face.normal * rng.uniform(.008, .022)
            center_id = len(vertices)
            vertices.append(tuple(center))
            faces.extend((indices[i], indices[(i + 1) % len(indices)], center_id)
    ...
    for polygon in mesh.polygons:
        ...
        if moss and polygon.center.z < 1.15 and n.z > -.1 and rng.random() < .09:
...
manifest = {
    ...
    'construction': 'Individually authored clipped asymmetric blocks, broad diagonal strata, one off-centre facet per large plane; no tapered ring mesh, loose strip, micro-noise or subdivision.',
