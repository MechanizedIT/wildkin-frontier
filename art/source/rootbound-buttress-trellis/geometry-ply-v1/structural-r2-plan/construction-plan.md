...
2. **Retain the restrained measured trunk S-bend.** The centerline is interpolated from actual 1.5-3.7 m horizontal-band centroids. Within the frozen 0.30 m full-weight XY core and 0.12 m fade (0.42 m outer radius), and Z 1.35-3.20 m, apply the literal XY control offsets in `parameters.json`: negative X through the middle, easing from -0.065 m at 2.5 m toward -0.035 m at 3.0 m. The upper fork is left unchanged because expanding this selection produced non-positive candidate normal dots. This retains a bounded S-bend without inventing fork stubs or moving ground vertices.
