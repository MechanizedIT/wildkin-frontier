...
const out='.dream-loop/overnight2-survey/package',browser=await chromium.launch({headless:true,channel:'msedge'}),page=await browser.newPage({viewport:{width:844,height:390}}),errors=[],steps=[];
page.on('pageerror',e=>errors.push(e.message));const external=[],failed=[],requests=[];page.on('request',r=>{requests.push(r.url());if(r.url().startsWith('http')&&new URL(r.url()).origin!=='http://localhost:8081')external.push(r.url());});page.on('requestfailed',r=>failed.push({url:r.url(),error:r.failure()}));
...
try{
 await page.goto('http://localhost:8081/');await page.locator('[data-action=start]').click({timeout:60000});
 ...
 await page.locator("[data-action=start]").click({timeout:60000});await page.context().setOffline(true);await walk(.3,24);assert.deepEqual(external,[]);assert.deepEqual(failed,[]);assert.deepEqual(errors,[]);await fs.writeFile(`${out}/wreck-native.json`,JSON.stringify({fixture:'Isolated fresh start. Actual keyboard travel from pod through Camp Travel/map, Forest Edge trail and west wreck entrance; actual chest and pack clicks. Read-only position-based navigation. No save seeds, teleport, grants, camera overrides or debug mutations.',steps,reward,loaded,errors,external,failed,requests,alreadyLoadedOfflineMovement:true,pass:true},null,2));console.log('Earned fresh Survey cartridge/west entry/exit/literal reload PASS');
