...
def trunk(name,rings,underbody=False):
    ...
    for station,(x,y,z,ry,rz,notch) in enumerate(rings):
        for deg in angles:
            ...
            if abs(x)<1.96 and deg in (-18,-9,0,9,18):
                # A shallow, irregular upper-front worked bite: its pale cap is
                # readable at play scale, while continuous bark remains below,
                # above, and around the rear of the same trunk.
                edge=deg/18
                qy=1.20+edge*(.21+.018*math.sin(x*6.4+deg))+.012*math.sin(x*3.1+deg)
                qz=1.025-notch*(.58+.42*(1-abs(edge)))
            vs.append((qx,qy,qz))
    faces.append(tuple(reversed(range(n))));cs.append(4)
    for i in range(len(rings)-1):
        for j in range(n):
            face=(i*n+j,i*n+(j+1)%n,(i+1)*n+(j+1)%n,(i+1)*n+j)
            notchface=(rings[i][-1] or rings[i+1][-1]) and j in (6,7,8,9)
            if notchface:
                # Pair of broad, offset sapwood facets, never a hardware-shaped
                # socket. The brighter inner plane makes the actual cut legible.
                faces.extend([(face[0],face[1],face[2]),(face[0],face[2],face[3])])
                cs.extend([4,12] if (i+j)%2 else [12,4])
            else:
                faces.append(face)
...
def seam():
    # Own pivot is ground level at (cutX,0,1.0). Small bark chips sit entirely
    # inside the pale cut silhouette; they are bark fragments, never fittings.
    outlines=[ [(-.29,1.15),(-.20,1.03),(-.08,1.07),(-.14,1.18),(-.26,1.25)],
        [(-.09,1.28),(.015,1.11),(.105,1.17),(.055,1.31),(-.025,1.36)],
        [(.12,1.18),(.22,1.07),(.30,1.13),(.24,1.24),(.17,1.28)] ]
    for i,poly in enumerate(outlines):
        # Thin pentagonal bark laminae peel along diagonal grain directions.
        # Their silhouette contains sharp tapered corners, never cuboid plugs.
        n=len(poly);verts=[(x,y,-.045) for x,y in poly]+[(x,y,-.012+.022*math.sin(j*1.7+i)) for j,(x,y) in enumerate(poly)]
        faces=[tuple(reversed(range(n))),tuple(range(n,2*n))]+[(j,(j+1)%n,(j+1)%n+n,j+n) for j in range(n)]
        mesh('RootfallBark_chunk_%d'%i,verts,faces,[1,0]+[1 if j%2 else 0 for j in range(n)])
...
manifest={'status':'candidate awaiting independent model review; no runtime admission',
 ...
 'revision':'V3: brighter irregular upper-front sapwood cuts, bark-only inset chips, and warm grounded central underbody facets',
 'cutWork':{'planesX':[-1.65,1.65],'frontBandY':[0.98,1.42],'recessDepthM':0.17,'finish':'two pale sapwood facets with three dark bark chips contained inside each notch'},
 'seams':{'model':'rootfall-seam.glb','positions':[[-1.65,0,1],[1.65,0,1]],'scale':1,'yaw':0,'pivot':'own ground origin','chunks':['RootfallBark_chunk_0','RootfallBark_chunk_1','RootfallBark_chunk_2'],'collisionEnabledRecommendation':False,'targetBox':{'size':{'w':.74,'h':1.35,'d':.25},'offset':{'x':0,'y':.675,'z':0}}},
