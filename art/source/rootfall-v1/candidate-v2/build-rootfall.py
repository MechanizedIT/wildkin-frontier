...
colors=[(96,70,51),(107,77,55),(115,83,59),(86,64,48),(166,129,83),(63,90,77),(78,105,81),(103,122,79),(112,119,108),(141,142,119),(120,95,60),(167,144,96)]
...
def trunk(name,rings):
    ...
    for station,(x,y,z,ry,rz,notch) in enumerate(rings):
        for deg in angles:
            ...
            if abs(x)<1.96 and deg in (-9,0,9):
                # The work area is independent of the major trunk taper.
                # Wavering top/bottom edges meet in a narrow wedge at the plane.
                qy=1.08+deg/9*(.14+.025*math.sin(x*6.4+deg))
                qz=1.03-(notch if deg==0 else 0)
            vs.append((qx,qy,qz))
    ...
    for i in range(len(rings)-1):
        for j in range(n):
            ...
            if notchface:
                # One exposed triangular tear, not a rectangular tan socket.
                faces.extend([(face[0],face[1],face[2]),(face[0],face[2],face[3])])
                cs.extend([4,0] if (j==7)==bool(rings[i][-1]) else [0,4])
            else:
                faces.append(face)
                moss=(j in (10,11,12) and abs(rings[i][0])>2 and (i+2*j)%5 not in (0,1))
                cs.append((5 if j%2 else 6) if moss else (1 if (i*3+j*2)%13==0 else 0))
...
def seam():
    ...
    outlines=[ [(-.34,1.09),(-.19,.97),(-.06,1.02),(-.12,1.1),(-.27,1.18)],
        [(-.13,1.09),(.015,.95),(.12,1.03),(.045,1.15),(-.025,1.24)],
        [(.08,1.14),(.22,1.01),(.35,1.08),(.25,1.14),(.19,1.22)] ]
    for i,poly in enumerate(outlines):
        ...
        mesh('RootfallBark_chunk_%d'%i,verts,faces,[4,0]+[4 if j%2 else 1 for j in range(n)])
...
manifest={'status':'candidate awaiting independent model review; no runtime admission',
 ...
 'revision':'V2: tapered integrated buttress, triangular front fractures/thin skin chips, continuous timber shaft lashings',
