...
def trunk(name,rings):
    ...
    for x,y,z,ry,rz,notch in rings:
        for deg in angles:
            ...
            rough=1+.055*math.sin(x*1.7+th*3)+.04*math.cos(x*.83-th*5)
            qz=z+rz*math.cos(th)*rough
            if deg==0:qz-=notch
            vs.append((x,max(0,y+ry*math.sin(th)*rough),qz))
    ...
    for i in range(len(rings)-1):
        for j in range(n):
            ...
            cs.append(4 if notchface else (0 if (i+j)%4 else 1 if j%3 else 3))
    ...
    for i in range(len(rings)-1):
        ...
        for j in (10,12):
            ...
            for k,v in enumerate(polygon):
                q=center+(v-center)*(.71 if k%2 else .94);q.y+=.022;patch.append(tuple(q))
                edge=(v+polygon[(k+1)%4])/2
                q=center+(edge-center)*(1.01 if k%2 else .66);q.y+=.027;patch.append(tuple(q))
            mesh('attached irregular moss bark shield',patch,[tuple(range(8))],5 if i%2 else 6)
    return vs
...
def seam():
    ...
    for i,(x,w,h) in enumerate([(-.18,.20,.18),(0,.18,.28),(.18,.17,.21)]):
        y=1.08+[.025,-.01,.045][i];z=-.005
        ...
        mesh('RootfallBark_chunk_%d'%i,verts,[(0,3,2,1),(4,5,6,7),(0,1,5,4),(1,2,6,5),(2,3,7,6),(3,0,4,7)],[4,0,4,0,4,1])
