...
p=argparse.ArgumentParser();p.add_argument('--output-dir',required=True);p.add_argument('--weights-only',action='store_true')
args=p.parse_args(sys.argv[sys.argv.index('--')+1:]);OUT=Path(args.output_dir).resolve();OUT.mkdir(parents=True,exist_ok=True)
ROOT=Path(__file__).resolve().parents[2];SOURCE=ROOT/'.dream-loop/overnight-tidefin-model/model-v5/final'
