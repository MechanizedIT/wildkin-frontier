...
The independent numerical foreground bounds are 1,149 px wide by 1,199 px high
(projected height-to-width `≈1.044`), so the complete visible silhouette is
roughly as wide as it is tall. At a 5.6 m target height, aim for an approximate
**4.8–5.8 m source-facing visible spread**. The exact front/depth relation is
not recovered from this perspective image: the deeper unseen mass remains an
inference inside the allowed 6.2 × 5.4 m target footprint and must be judged in
all-angle massing renders. The footprint is an envelope, not a license to make
a 7 m flat front silhouette.
...
3. **Three joined major structural directions.** Build forks by splitting and
   ...
   Under the declared matching source-facing review framing, keep the irregular
   fork window within the observed normalized envelope `x≈0.38–0.65`,
   `y≈0.39–0.58`; compare its screen-space opening and supported limb edges,
   rather than claiming an uncalibrated meter size from the target camera. The
   branch system must remain visibly continuous in the rear and three-quarter
   renders; add only short, subordinate support stubs where a canopy lobe
   otherwise floats.
...
renders of the actual fresh mesh. The source-facing three-quarter must show a
broad continuous grounded base with several readable toe termini, the trunk's
rightward lean, the left/right/upper structural directions, three separate
canopy groups, and the central negative-space window. The rotating
front/rear/left/right/three-quarter set must establish all five distinct closed
buttresses without tunnels, substantial depth, real fork attachments, and a
closed root base. A flat front cutout, a radial starburst, or canopy volumes
