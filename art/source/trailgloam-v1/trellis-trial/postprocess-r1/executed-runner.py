...
def launch(cmd,install):
 flags=getattr(subprocess,'CREATE_NO_WINDOW',0)
 return subprocess.Popen(cmd,cwd=str(install),env=offline_env(install),creationflags=flags)
...
def native_phase_trace(torch, output, install):
 ...
 hooks={114:'cumesh-create',129:'bvh-create',194:'remesh',219:'simplify',258:'uv',297:'rasterize',324:'bake-bvh',419:'return'}
 samples=[]
 def trace(frame,event_name,arg):
  if event_name=='line' and Path(frame.f_code.co_filename).resolve()==source and frame.f_lineno in hooks:
...
def main():
 ...
 try:
  for child in ('decode','postprocess'):
   ...
   q=launch(cmd,a.install_root); code=q.wait()
   if code:
