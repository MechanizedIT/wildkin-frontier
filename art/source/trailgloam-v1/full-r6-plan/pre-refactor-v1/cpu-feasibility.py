...
def main():
 d=json.loads(P.read_text()); shell=r5.ring_loft(d['parts']['shell']['rings_z_rx_ry'],d['parts']['shell']['sides'],'saucer'); head=r5.prism(d['parts']['head']['start'],d['parts']['head']['end'],d['parts']['head']['radius_start'],d['parts']['head']['radius_end'],d['parts']['head']['sides'],'head'); eyes=[r5.prism([x,-.72,.53],[x,-.82,.52],.055,.040,6,f'eye_{side}') for x,side in ((-.13,'L'),(.13,'R'))]; parts=[shell,head]+eyes; contacts=[]
 for leg in d['legs']:
