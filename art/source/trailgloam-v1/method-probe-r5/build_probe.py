...
def main():
    ...
    for i,point in enumerate((anchors[1],anchors[2],anchors[3])):
        # Each interior cuff follows its own two adjoining links.  Do not let
        # the terminal ankle tangent contaminate the coxa or knee bisector.
        direction=unit(add(tangents[i],tangents[i+1])) if i < 2 else tangents[-1]
    ...
    contacts={
        ...
        'cuff_2_complete_lower_cap_ring_inside_hoof':contains(hoof,cuffs[2]['rings']['end'])
    }
    ...
    proof={"schema":"trailgloam-method-probe-r5-cpu-proof-v1","parameters_sha256":sha(args.params),"source":{"reference":params['reference'],"axis":params['axis']},"scope":"one +X representative chain and one frond only; not a model candidate","host_global_convexity_from_generated_face_halfspaces":host_convex,"parts":audits,"cuff_axis_inputs":{"cuff_0":{"links":[0,1],"axis":cuffs[0]['axis']},"cuff_1":{"links":[1,2],"axis":cuffs[1]['axis']},"cuff_2":{"links":[2],"axis":cuffs[2]['axis']}},"contacts":contacts,"hoof_min_z":min(v[2] for v in hoof['vertices']),"complete_bounds":bounds,"all_parts_closed":all(row['boundary_edges']==0 and row['nonmanifold_edges']==0 and row['degenerate_faces']==0 and row['finite'] and row['outward_centroid_dot_min']>0 for row in audits.values()),"all_measured_contacts_pass":all(row['contained'] for row in contacts.values()),"limits":params['limits']}
