import test from 'node:test';import assert from 'node:assert/strict';import fs from 'node:fs';
import {applyFenBankSurfaceAndGrounding,composeShatterfenBank,FEN_BANK_OUTCROPS,FEN_BANK_COMPONENTS,getFootSamples} from '../tools/compose-shatterfen-bank.mjs';
import {composeShatterfenUplands} from '../tools/compose-shatterfen-uplands.mjs';
import {getSurfaceHeight,getRouteDistance,validateSurface} from '../src/world/terrainSurfaceModel.js';
import {getGravelCoverage,getGravelCanvasSize,paintGravelRoute} from '../src/presentation/gravelRoutePaint.js';
import {createAuthoredTerrain} from '../src/presentation/authoredTerrain.js';
const baseline=JSON.parse(fs.readFileSync(new URL('../src/world/data/world.json',import.meta.url),'utf8'));
test('bank composition preserves gameplay, terrain support and sibling regions; rerun is stable',()=>{
 const world=structuredClone(baseline),before=structuredClone(world),old=before.regions.find(r=>r.id==='section_2');composeShatterfenBank(world);const bank=world.regions.find(r=>r.id==='section_2');
 assert.deepEqual(world.regions.filter(r=>r.id!=='section_2'),before.regions.filter(r=>r.id!=='section_2'));
 for(const field of Object.keys(old).filter(k=>!['props','surface'].includes(k)))assert.deepEqual(bank[field],old[field],field);
 assert.deepEqual(world.visualAssets,before.visualAssets,'existing bank asset and frond recipes are not regenerated or rewritten');
 const fixed=p=>!p.id.startsWith('prop_fen_bank_')&&p.id!=='prop_s2_observatory_stone_r';assert.deepEqual(bank.props.filter(fixed),old.props.filter(fixed));
 for(const key of ['heights','water','palette','detail'])assert.deepEqual(bank.surface[key],old.surface[key],key);
 for(let x=12;x<=35;x+=.43)for(let z=0;z<=20;z+=.47)assert.equal(getSurfaceHeight(bank.surface,x,z),getSurfaceHeight(old.surface,x,z));
 for(const route of old.surface.routes.filter(r=>!r.id.startsWith('fen_bank_')))assert.deepEqual(bank.surface.routes.find(r=>r.id===route.id),route);
 for(const route of bank.surface.routes.filter(r=>r.id.startsWith('fen_bank_')))assert.equal(route.scatter,false,`${route.id} remains a local paint-only footprint`);
 assert.deepEqual(bank.surface.routes.filter(r=>r.id.startsWith('fen_bank_')).map(r=>r.id),['fen_bank_approach','fen_bank_apron','fen_bank_receiver','fen_bank_resource_skirt']);
 const once=structuredClone(world);composeShatterfenBank(world);assert.deepEqual(world,once);validateSurface(bank.surface);
});
test('bank helper accepts the legacy pre-uplands Author route as well as the V2 forecourt',()=>{
 const world=structuredClone(baseline),bank=world.regions.find(r=>r.id==='section_2'),forecourt=bank.surface.routes.find(r=>r.id==='fen-observatory-forecourt');
 assert.ok(forecourt,'current fixture contains the V2 route');
 bank.surface.routes=bank.surface.routes.map(route=>route.id==='fen-observatory-forecourt'?{...structuredClone(route),id:'observatory-island-spur'}:route);
 assert.doesNotThrow(()=>composeShatterfenBank(world),'the pre-uplands Author order remains valid');
 assert.ok(bank.surface.routes.some(route=>route.id==='observatory-island-spur'),'legacy support route is preserved');
 assert.deepEqual(bank.surface.routes.filter(route=>route.id.startsWith('fen_bank_')).map(route=>route.id),['fen_bank_approach','fen_bank_apron','fen_bank_receiver','fen_bank_resource_skirt']);
});
test('solid outcrops avoid protected approach/resource points and their complete feet are buried',()=>{
 const world=composeShatterfenBank(structuredClone(baseline)),bank=world.regions.find(r=>r.id==='section_2');
 const points=[{x:19,z:11,r:1.6},{x:23,z:8,r:1.65},{x:23,z:6.34,r:.3},{x:27,z:12,r:1.7},{x:29,z:18,r:2.2},{x:20.9,z:9.5,r:1.1}];
 for(const s of FEN_BANK_OUTCROPS)for(const p of points){const gap=Math.hypot(Math.max(Math.abs(p.x-s.x)-s.w/2,0),Math.max(Math.abs(p.z-s.z)-s.d/2,0));assert.ok(gap>p.r,`${s.name} infringes ${JSON.stringify(p)}`);}
 for(const s of FEN_BANK_COMPONENTS){const prop=bank.props.find(p=>p.id===`prop_fen_bank_${s.id}`),asset=world.visualAssets.find(a=>a.id===prop.visualAssetId),samples=getFootSamples(s,asset.collision);assert.ok(samples.length>100);for(const [x,z]of samples)assert.ok(prop.pos.y<getSurfaceHeight(bank.surface,x,z));}
});
test('upland terrain replacement reapplies bank paint and full-foot grounding without touching admitted asset metadata',()=>{
 const world=structuredClone(baseline),beforeAssets=structuredClone(world.visualAssets);composeShatterfenUplands(world);const bank=world.regions.find(r=>r.id==='section_2');
 assert.deepEqual(world.visualAssets,beforeAssets,'narrow bank post-terrain helper does not rewrite assets');
 for(const id of ['fen_bank_approach','fen_bank_apron','fen_bank_receiver','fen_bank_resource_skirt']){
  const route=bank.surface.routes.find(r=>r.id===id);assert.ok(route,`${id} is reapplied after terrain replacement`);assert.equal(route.scatter,false);assert.equal(route.elevation,undefined,'paint-only route does not alter terrain height');
 }
 for(const s of FEN_BANK_COMPONENTS){const prop=bank.props.find(p=>p.id===`prop_fen_bank_${s.id}`),asset=world.visualAssets.find(a=>a.id===prop.visualAssetId);for(const [x,z]of getFootSamples(s,asset.collision))assert.ok(prop.pos.y<getSurfaceHeight(bank.surface,x,z),`${s.id} foot stays buried after terrain rebase at ${x},${z}`);}
 // Repeating only the narrow helper cannot introduce duplicate paint paths or
 // drift the measured support bases.
 const once=structuredClone(bank);applyFenBankSurfaceAndGrounding(world);assert.deepEqual(bank,once);
});
test('gravel mask remains walk-route paint with irregular feathering and validated opt-in',()=>{
 const route={points:[{x:0,z:0},{x:0,z:12}],width:3.4,style:'gravel'};assert.ok(getGravelCoverage(route,0,6)>.5);assert.equal(getGravelCoverage(route,8,6),0);const edge=[1,2,3,4,5,6,7].map(z=>getGravelCoverage(route,1.8,z));assert.ok(Math.max(...edge)-Math.min(...edge)>.1);assert.ok(edge.every(v=>v>=0&&v<=1));
 validateSurface({routes:[route]});assert.throws(()=>validateSurface({routes:[{...route,style:'unknown'}]}),/path style/);assert.equal(getSurfaceHeight({routes:[route]},0,6),0);
 assert.throws(()=>validateSurface({routes:[{...route,scatter:'no'}]}),/scatter flag/);
 assert.deepEqual(getGravelCanvasSize(20,8),{width:520,height:208});const large=getGravelCanvasSize(418,210);assert.equal(large.width,768);assert.ok(Math.abs(large.height/large.width-210/418)<1/768);
});
test('bank masks remove only local meadow instances and never reseed distant foliage',()=>{
 const world=structuredClone(baseline),old=structuredClone(world.regions.find(r=>r.id==='section_2'));old.surface.routes=old.surface.routes.filter(r=>!r.id.startsWith('fen_bank_'));
 composeShatterfenBank(world);const next=world.regions.find(r=>r.id==='section_2'),before=createAuthoredTerrain(old).group,after=createAuthoredTerrain(next).group;
 const bankRoutes=next.surface.routes.filter(r=>r.id.startsWith('fen_bank_'));
 for(const name of ['meadow_grass','meadow_ferns']){const collect=root=>{const mesh=root.getObjectByName(name),map=new Map();for(let i=0;i<mesh.count;i++){const m=Array.from(mesh.instanceMatrix.array.slice(i*16,i*16+16));map.set(JSON.stringify(m),{x:m[12],z:m[14]});}return map;};const a=collect(before),b=collect(after);for(const key of b.keys())assert.ok(a.has(key),'new distant tuft was introduced');for(const [key,pos]of a)if(!b.has(key))assert.ok(Math.min(...bankRoutes.map(route=>getRouteDistance(route,pos.x,pos.z)))<.301,'removed tuft is outside the local bank paint clearing');}
});

test('an authored gravel tint changes baked colour without moving its mask or changing the default',()=>{
 const previous=globalThis.document,images=[];
 globalThis.document={createElement(){return{width:0,height:0,getContext(){return{createImageData(w,h){return{data:new Uint8ClampedArray(w*h*4)};},putImageData(image){images.push(image.data);}};}};}};
 const route={points:[{x:0,z:0},{x:0,z:5}],width:3.4,style:'gravel'},ctx={drawImage(){}};
 try{
  paintGravelRoute(ctx,route);paintGravelRoute(ctx,route,'#979482');paintGravelRoute(ctx,route,'#ad844f');
  assert.deepEqual(images[0],images[1],'legacy default stays byte identical');
  let changed=0;for(let i=0;i<images[0].length;i+=4){assert.equal(images[0][i+3],images[2][i+3],'tint does not change path coverage');if(images[0][i+3]>240){assert.ok(images[2][i]>images[0][i]&&images[2][i+2]<images[0][i+2],'warm centre replaces the cool gravel base');changed++;}}
  assert.ok(changed>100,'substantial visible path interior is recoloured');
  const surface={routes:[route],palette:{gravel:'#ad844f'}};assert.deepEqual(validateSurface(JSON.parse(JSON.stringify(surface))),surface);
  assert.throws(()=>validateSurface({...surface,palette:{gravel:'warm'}}),/palette color/);
 }finally{if(previous===undefined)delete globalThis.document;else globalThis.document=previous;}
});
