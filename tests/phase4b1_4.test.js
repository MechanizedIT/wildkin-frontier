...
describe("Phase 4B.1.4 — canonical Author parity", () => {
  ...
  it("keeps real structures player-facing and technical markers editor-only", () => {
    ...
    const draft = createAuthorDraft(WORLD_DATA);
    for (const id of ["gate_section_1_camp_arrival", "gate_section_1_to_2", "wp_section_1", "beacon_section_1"]) {
    ...
    for (const id of ["jump_pad_section_1", "parkour_start_section_1", "parkour_checkpoint_section_1", "parkour_end_section_1", "kill_volume_section_1"]) {
      assert.equal(findAuthorRoot(staticWorld.group, id), null, `${id} must not return to Play from a legacy fixture`);
    }
    for (const id of ["entry_section_1"]) {
