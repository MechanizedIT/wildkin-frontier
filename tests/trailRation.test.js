...
function fixture(run){
 ...
 try{const progress=createFrontierProgress({resourceDrops:WORLD_DATA.resourceDrops});progress.load();const supplies={wood:6,stone:3,berries:8,fiber:6};assert.deepEqual(progress.collectResources(supplies).added,supplies);assert.equal(progress.getInventoryState().pack.length,16);let health=3,active=true,feedback=0;
...
test('ration recipe uses existing station, ingredient and persistent stack/loadout transactions',()=>fixture(({progress:p})=>{
 ...
 assert.equal(p.placeStructure(bedlessBench).placed,true);const pack=p.getPackResourceCounts();assert.equal(p.craftFieldSupply(item.id).crafted,true);
 assert.equal(p.getPackResourceCounts().berries,pack.berries-2);assert.equal(p.getPackResourceCounts().fiber,pack.fiber-1);assert.equal(p.getFieldSupplies().berry_lure,0);
