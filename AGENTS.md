...
> **Current product direction (September 12, 2026):** Chris approved the Living Frontier pivot and continuous goal-mode development directly on main: procedural geography/ecology, individual modular Wildkin, immersive Camp building/care/research/breeding and later DNA recipes. Latest steering makes portrait mobile/casual first, with landscape and desktop support. No legacy branch or backwards compatibility requirement. Read `docs/LIVING_FRONTIER_PLAN.md` and `docs/CURRENT_SLICE.md`; preserve explicit ownership and focused proof. Exact design/art remains provisional through play. Chris authorized the next GitHub backup with refreshed reports and relevant documentation included.

> **September 13 stop:** Chris requested a PC restart and work/workflow review. The final continent-map and safe character-flight requests are complete. Read `docs/SESSION_HANDOFF_2026-09-13.md` and await fresh direction; do not automatically resume the earlier continuous mandate. Previous GitHub/Drive publication authorization was fulfilled at its recorded checkpoint.

1. Read `docs/CURRENT_SLICE.md` first. It is the only authoritative implementation scope for this session.
   Then read `docs/SESSION_START.md` and the latest restart handoff. After fresh owner resumption, use `docs/LIVING_FRONTIER_PLAN.md` and relevant `docs/CODE_MAP.md` entries. Use the Wildkin skill and Dream Loop for visual work, normally capped at three improvement passes before choosing the strongest usable result. Emberfall remains an isolated candidate; read its handoff before integration.
...
4. Preserve hard constraints: single-player, portrait-mobile/casual first with landscape and desktop support (latest September12 owner direction), Three.js/HTML5, no external runtime network requests, local vendored `vendor/three.module.js` + `vendor/rapier.js` with relative paths, `index.html` at ZIP root, readable first-party source. The old35MB cap is retired. Report size and keep rendering/loading costs deliberate.
