...
1. Read `docs/CURRENT_SLICE.md` first. It is the only authoritative implementation scope for this session.
   Then use `docs/SESSION_START.md` for the short handoff and `docs/CODE_MAP.md` to find relevant owners. Chris requested the September 11 overnight run wrap up for his playtest; do not automatically resume the old all-night mandate. Await or act on his new feedback. Use the project workflow in `.agents/skills/wildkin-development/SKILL.md` for subsequent development; the overnight mandate is retained as historical owner direction.
