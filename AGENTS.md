...
1. Read `docs/CURRENT_SLICE.md` first. It is the only authoritative implementation scope for this session.
   Then read `docs/LIVING_FRONTIER_PLAN.md`, `docs/SESSION_START.md` and `docs/CODE_MAP.md`. Chris explicitly resumed after the restart handoff and authorized bounded iteration until he asks to stop. Use the Wildkin skill and Dream Loop for visual work, normally capped at three improvement passes before choosing the strongest usable result. Emberfall remains an isolated candidate; read its handoff before integration.
