...
1. Read `docs/CURRENT_SLICE.md` first. It is the only authoritative implementation scope for this session.
   Then use `docs/SESSION_START.md` and `docs/SESSION_HANDOFF_2026-09-12.md` for the restart handoff, and `docs/CODE_MAP.md` to find relevant owners. Chris requested stopping on September12 for a PC restart and a new Codex session. Do not automatically resume the old all-night mandate; act on a new explicit resume or feedback. Use `.agents/skills/wildkin-development/SKILL.md`. The isolated Emberfall candidate is not runtime content; read its handoff before integration.
...
4. Preserve hard constraints: single-player, landscape-first mobile/web (Chris explicitly replaced portrait on September 10, 2026), Three.js/HTML5, no runtime network requests, local vendored `vendor/three.module.js` + `vendor/rapier.js` with relative paths, `index.html` at ZIP root, first-party code readable/unminified in submission. Chris explicitly retired the old hackathon 35 MB package cap on September 10, 2026. Report package size without enforcing that obsolete limit; keep mobile rendering and loading costs deliberate.
