...
4. Preserve hard constraints: single-player, portrait mobile, Three.js/HTML5, no runtime network requests, local vendored `vendor/three.module.js` + `vendor/rapier.js` with relative paths, `index.html` at ZIP root, first-party code readable/unminified in submission. Chris explicitly retired the old hackathon 35 MB package cap on September 10, 2026. Report size without enforcing that obsolete limit; keep mobile rendering and loading costs deliberate.
