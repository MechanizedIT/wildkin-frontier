...
4. Preserve hard constraints: single-player, PC/web playable first with landscape mobile viability and portrait fallback (September12 owner direction), Three.js/HTML5, no runtime network requests, local vendored `vendor/three.module.js` + `vendor/rapier.js` with relative paths, `index.html` at ZIP root, readable first-party source. The old35MB cap is retired. Report size and keep rendering/loading costs deliberate.
...
6. Keep the build playable at the end of every session — no console-breaking errors, landscape mobile layout usable with a responsive portrait fallback, offline-safe.
