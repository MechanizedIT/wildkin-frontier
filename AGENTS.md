...
1. Read `docs/CURRENT_SLICE.md` first. It is the only authoritative implementation scope for this session.
   Then use `docs/SESSION_START.md` for the short handoff and `docs/CODE_MAP.md` to find relevant owners. Chris explicitly resumed overnight goal-mode work after his September11 evening phone playtest; `docs/OVERNIGHT_RUN_2.md` records the active mandate and feedback. Continue until he says stop, inside the currently selected bounded slice. Use `.agents/skills/wildkin-development/SKILL.md`; earlier wrap-up instructions and the first overnight chronology are historical where superseded.
