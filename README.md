...
- desktop `?author=1` Author Mode: transactional draft + 40-step undo/redo, canonical descriptor for Edit/Play/Rapier parity, live Ground/Boundary proxy, 21 searchable/categorized starter Visual Assets, isolated Asset Workbench with 45° camera steps plus fixed-pitch dolly/camera-plane pan, compact ordered-part controls (`Space`/`C` vertical nudge and `Q`/`E` rotation), simple Box collision, per-recipe Prop/Harvestable/Wildkin roles, custom depleted remnants and pickup models, Visual Asset Waypoint/Beacon models, unbounded world-object drag, draft-derived extents, region/ownership validation, selection-driven hierarchy disclosure, bounded shortcuts (ignored while typing), capability-aware Visible/Collision/Opacity/Tint,
...
Visual Asset workflow: use search/categories to find a starter recipe, then **Place** or **Edit**. The focused Asset Workbench hides the rest of the scene and defers shared world-instance rebuilding until exit. Select a part and use WASD/arrows for local X/Z, `Space`/`C` for local Y, `Q`/`E` for local Y rotation, and `Shift` for coarse steps. Use `[`/`]` (or Camera View buttons) to orbit 45°, `0` to reset, right-drag to pan in the camera plane (vertical pointer direction is inverted), and the wheel to dolly at fixed pitch. Reorder parts vertically; Harvestables remove the bottom entry first. Set **Game Object Type** to Prop, Harvestable, or Wildkin. Harvestables can choose/create drops, assign pickup/remnant Visual Assets, hits, respawn, and impact feel. Wildkin expose the existing behavior/temperament/combat/spatial settings. Waypoints/Beacons can choose a Visual Asset under **World Model**. All instances of a recipe share its role.
