...
export function chooseNearbyInteraction({ camp, gate, loot, frontier, field, activeTamingId, isVisible = () => true }) {
  const activeField=field?.type==='bond'&&field.id===activeTamingId?field:null;
  return [camp,gate,loot,activeField,frontier,field].find(info=>info&&isVisible(info))??null;
}
