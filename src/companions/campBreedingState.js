...
export const CAMP_BREEDING_GUIDED_TRAITS = Object.freeze(['baseColor']);
...
export function cloneCampBreeding(breeding) {
  return breeding === null ? null : {
    ...
    guidedTrait: breeding.guidedTrait ?? null,
...
export function createCampOffspring(parentA, parentB, sequence, guidedTrait = null) {
  ...
  if (!a || !b || a.id === b.id || a.speciesId !== 'mossling' || b.speciesId !== 'mossling' || a.sex === b.sex
    || (guidedTrait !== null && !CAMP_BREEDING_GUIDED_TRAITS.includes(guidedTrait))) return null;
  ...
  return normalizeWildkinIndividual({
    ...
    genome: inheritWildkinGenome(a.genome, b.genome, `camp-breed:${sequence}:${a.id}:${b.id}`,
      guidedTrait ? { preserveTrait: guidedTrait } : undefined),
  });
}
...
export function readCampBreeding(value, { structures = [], ownedWildkin = [], observationClues = {} } = {}) {
  if (value === null || value === undefined) return { ok: true, breeding: null };
  const keys = ['version', 'bedId', 'parentIds', 'guidedTrait', 'offspring', 'growthSeconds'];
  const shape = value && typeof value === 'object' && !Array.isArray(value)
    && Object.keys(value).every(key => keys.includes(key));
  const guidedTrait = shape ? value.guidedTrait ?? null : null;
  ...
  if (!shape || value.version !== CAMP_BREEDING_VERSION || !validId(value.bedId)
    ...
    || (guidedTrait !== null && !CAMP_BREEDING_GUIDED_TRAITS.includes(guidedTrait))
  ...
  if (guidedTrait && observationClues.mossling !== 2) return { ok: false, reason: 'invalid-camp-breeding-guidance' };
  ...
  const expectedOffspring = offspringSequence === null ? null : createCampOffspring(parents[0], parents[1], offspringSequence, guidedTrait);
  ...
  return { ok: true, breeding: cloneCampBreeding({ ...value, guidedTrait, offspring }) };
}
