...
export function createAuthorDraft(repoData) {
  ...
  function findObjectById(id) {
    // Search across all region collections: props, platforms, obstacles, climbables, resources, creatures, waypoints, beacons, pois
    ...
    // Also check camp region? Already included. Camp itself is not in regions but could be searched?
    return null;
  }
  ...
  function updateTransform(id, patch) {
    ...
    if (patch.regionId && patch.regionId !== found.region.id) {
      const targetRegion = findRegion(patch.regionId);
      if (!targetRegion) return { ok: false, error: "target region not found" };
      // Move object between regions — validate pos inside target bounds? Will be validated later, but keep.
      // Remove from old
    ...
    if (patch.displayName !== undefined && found.collection === "regions") {} // handled separate
    persist();
    return { ok: true };
  }
  ...
  function createObject(regionId, kind, subtype) {
    ...
    let obj = null;
    let collection = null;
    if (kind === "box" || kind === "prop") {
      collection = "props";
      const id = nextId(subtype || "prop");
      obj = { id, subtype: subtype || "box", pos: { x: centerX, y: 0, z: centerZ }, size: { w: 1.5, h: 1.0, d: 1.5 }, rotY: 0 };
      ...
      collection = "majorWaypoints";
      const id = nextId("wp");
      obj = { id, type: "majorWaypoint", pos: { x: centerX, y: 0, z: centerZ } };
      region.majorWaypoints.push(obj);
    } else if (kind === "extractionBeacon") {
      collection = "extractionBeacons";
      const id = nextId("beacon");
      obj = { id, type: "extractionBeacon", pos: { x: centerX, y: 0, z: centerZ } };
      ...
      collection = "props";
      const id = nextId(kind);
      obj = { id, subtype: kind, pos: { x: centerX, y: 0, z: centerZ }, size: { w: 2, h: 1, d: 2 }, rotY: 0 };
      region.props.push(obj);
    } else {
