...
export function createSkillTree({ skills = {}, iconMarkup = fallbackIcon, selectedId = null } = {}) {
 ...
 // A branch keeps one vertical lane on phones. The former sideways child fan
 // made neighbouring branches overlap, leaving several skills hard to read or tap.
 roots.forEach((root,index)=>{const x=(index+.5)/Math.max(roots.length,1)*100;layout.set(root.id,{x,y:11});const kids=children(root.id);kids.forEach((kid,k)=>{layout.set(kid.id,{x,y:k===0?35:59});const tails=children(kid.id);tails.forEach(tail=>layout.set(tail.id,{x,y:84}));});});
 ...
 const detail=selected?`<article class="skill-detail is-${state}"><div class="skill-detail__icon">${iconMarkup(selected.icon||"skills",{size:52,label:selected.name})}</div><div><p class="beta-kicker">${esc(selected.branch||"MASTERY")} · LV ${selected.minLevel||1}</p><h3>${esc(selected.name)}</h3><p>${esc(selected.description)}</p><strong>${esc(lock)}</strong></div>${canBuy?`<button class="beta-gold-button" data-action="purchaseSkill" data-id="${esc(selected.id)}">AWAKEN</button>`:""}</article>`:"";
 const mobileList=nodes.map(n=>{const nodeState=statusFor(n,unlocked,points,level);return `<button class="skill-list__node is-${nodeState} ${selected?.id===n.id?"is-selected":""}" data-skill-id="${esc(n.id)}" aria-pressed="${selected?.id===n.id}">${iconMarkup(n.icon||"skills",{size:30,label:n.name})}<span><b>${esc(n.name)}</b><small>${esc(n.branch)} · LV ${n.minLevel}</small></span>${unlocked.has(n.id)?"<em>✓</em>":"<i>›</i>"}</button>`;}).join("");
 return `<section class="skill-board"><div class="skill-board__top"><div><p class="beta-kicker">FRONTIER MASTERY</p><h2>Skill paths</h2></div><div class="skill-board__points">${iconMarkup("skills",{size:29,label:"Skill points"})}<b>${points}</b><small>POINTS</small></div></div>${detail}<div class="skill-tree"><svg class="skill-tree__lines" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">${lines}</svg>${buttons}</div><div class="skill-list">${mobileList}</div></section>`;
}
