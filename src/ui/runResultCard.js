...
export function createRunResultCard(opts = {}) {
  ...
  function show(data){
    ...
    const items=Object.entries(secured?{...cargo,xp}:{xp}).filter(([,n])=>n>0);
    body.innerHTML=`${resultCompanionsMarkup(companions,secured)}<p>${secured?'Your haul is in your backpack. Unload it into the pod locker or a storage crate.':'Backpack kept. Carried XP and unsecured bonds were lost.'}</p><div class="reward-grid ${secured?'secured':'lost'}">${items.map(([id,n])=>`<div>${iconMarkup(id,{size:46})}<b>${id==='xp'&&secured?'+':''}${n}</b></div>`).join('')}</div>${waypoints.length+beacons.length?`<p class="result-progress">${iconMarkup('map',{size:26})} ${waypoints.length+beacons.length} new discoveries${secured?'':' kept'}</p>`:''}${complete?'<p class="result-milestone">Heartwood is alive again.</p>':''}${secured&&data.upgradeAvailable?'<p class="result-progress">Workshop upgrade available</p>':''}`;
