...
export function createBetaShell({ app, getModel, onAction = () => null, onBlockingChanged = () => {}, canOpen = () => true, openInventory = null } = {}) {
  ...
  function companionPanel(model) {
    ...
    return `<section class="wildkin-panel"><div class="wildkin-heading"><h2>Wildkin journal</h2><span>${(model.pendingCompanions ?? []).length}/${model.captureCapacity ?? 1} pending bonds</span></div>
      <div class="wildkin-list" role="group" aria-label="Wildkin species">${companions.map(c => `<button data-companion-id="${esc(c.id)}" aria-pressed="${selected?.id === c.id}" class="wildkin-chip ${selected?.id === c.id ? "is-selected" : ""} ${c.secured ? "is-secured" : ""}" style="--wildkin:${esc(c.color || "#36c896")}">${icon(c.speciesId ?? c.id, { size: 30, label: c.name })}<span>${esc(c.name)}</span>${c.secured ? '<b aria-label="Secured">✓</b>' : ""}</button>`).join("")}</div>
