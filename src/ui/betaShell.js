...
export function createBetaShell({ app, getModel, onAction = () => null, onBlockingChanged = () => {}, canOpen = () => true, openInventory = null } = {}) {
  ...
  function open(tab = "journal") { if (!canOpen()) return false; if (fieldToolPointerId !== null) suppressOpeningClick = true; endFieldTool(); if((tab === 'inventory' || tab === 'equipment') && openInventory){close();return openInventory();} welcome = false; opened = true; activeTab = tab; if (tab === "inventory") inventoryTab = "equipment"; modal.hidden = false; root.classList.add("is-open"); setBlocking(true); signature = ""; render(true); focusModal(); return true; }
  ...
  function companionPanel(model) {
    ...
    return `<section class="wildkin-panel"><div class="wildkin-heading"><h2>Wildkin journal</h2><span>${(model.pendingCompanions ?? []).length}/${model.captureCapacity ?? 1} pending bonds</span></div>
      <div class="wildkin-list" role="group" aria-label="Wildkin species">${companions.map(c => `<button data-companion-id="${esc(c.id)}" aria-pressed="${selected?.id === c.id}" class="wildkin-chip ${selected?.id === c.id ? "is-selected" : ""} ${c.secured ? "is-secured" : ""}" style="--wildkin:${esc(c.color || "#36c896")}">${icon(c.id, { size: 30, label: c.name })}<span>${esc(c.name)}</span>${c.secured ? '<b aria-label="Secured">✓</b>' : ""}</button>`).join("")}</div>
      ${selected ? `<article class="wildkin-detail" style="--wildkin:${esc(selected.color || "#36c896")}"><div class="wildkin-detail__heading">${icon(selected.id, { size: 40, label: selected.name })}<div><p class="beta-kicker">${status}</p><h3>${esc(selected.name)}</h3></div></div><p class="wildkin-habitat">${esc(selected.secured ? selected.abilityDescription : selected.habitat || "Follow its frontier signal.")}</p>${!selected.secured && selected.taming ? `<div class="wildkin-taming"><strong>How to tame</strong><p class="wildkin-taming-guide">${esc(selected.taming.guide)}</p><small>Gear: ${esc(getFieldCraftLocation(selected.taming.supply))}.</small></div>` : ""}<section class="wildkin-field-notes" aria-label="Field notes"><h4>Field notes</h4>${selected.journalEntries?.length ? selected.journalEntries.map(note=>`<article><strong>${esc(note.title)}</strong><p>${esc(note.text)}</p></article>`).join("") : "<p>Watch quietly in the field to learn its habits.</p>"}${!selected.secured && (selected.journalEntries?.length??0)<2 ? `<small>${selected.journalEntries?.length??0}/2 notes · Keep watching to learn more.</small>` : ""}</section>${selected.secured ? `<button data-action="selectCompanion" data-id="${esc(selected.id)}">${selected.active ? "ACTIVE" : "SELECT"}</button>` : ""}</article>` : ""}</section>`;
  }
