...
export function createBetaShell({ app, getModel, onAction = () => null, onBlockingChanged = () => {}, canOpen = () => true } = {}) {
  ...
  function companionPanel(model) {
    ...
    return `<section class="wildkin-panel"><div class="wildkin-heading"><h2>Wildkin journal</h2><span>${(model.pendingCompanions ?? []).length}/${model.captureCapacity ?? 1} pending bonds</span></div>
      ...
      ${selected ? `<article class="wildkin-detail" style="--wildkin:${esc(selected.color || "#36c896")}"><div class="wildkin-detail__heading">${icon(selected.id, { size: 40, label: selected.name })}<div><p class="beta-kicker">${status}</p><h3>${esc(selected.name)}</h3></div></div><p class="wildkin-habitat">${esc(selected.secured ? selected.abilityDescription : selected.habitat || "Follow its frontier signal.")}</p>${!selected.secured && selected.taming ? `<div class="wildkin-taming"><strong>How to tame</strong><p class="wildkin-taming-guide">${esc(selected.taming.guide)}</p><small>Field gear: Camp → Work → Craft.</small></div>` : ""}${selected.secured ? `<button data-action="selectCompanion" data-id="${esc(selected.id)}">${selected.active ? "ACTIVE" : "SELECT"}</button>` : ""}</article>` : ""}</section>`;
  }
