...
import { EQUIPMENT_BY_ID, getEquipmentCount } from '../equipment/equipmentCatalog.js';
...
import { createFullscreenControl } from './fullscreenControl.js';
...
// Presentation only: quantities come from the selected physical source view.
export function ingredientGuidance(cost, costLabel, available = {}, cargo = {}, isCamp = true) {
  const missing=Object.entries(cost).filter(([id,count])=>(available[id]??0)<count).map(([id,count])=>String(count-(available[id]??0))+' '+label(id));
...
export function createBetaShell({ app, getModel, onAction = () => null, onBlockingChanged = () => {}, canOpen = () => true, openInventory = null } = {}) {
  ...
  hud.innerHTML = `<div class="beta-objective"></div><div class="beta-hud-actions equipment-toolbar" aria-label="Equipment quick slots"><span class="equipment-selected-name"></span><div id="portrait-tool-belt" class="equipment-slots"></div><button class="portrait-equipped beta-hud-button" data-action="togglePortraitTools" aria-controls="portrait-tool-belt" aria-expanded="false" aria-label="Change equipped item"></button><button class="beta-hud-button" data-action="open" data-tab="inventory" aria-label="Open backpack">${icon("backpack")}<span>PACK</span><b class="beta-cargo-count">0</b></button></div><div class="beta-quick" aria-label="Companion ability"><button data-action="ability" class="beta-ability">${icon("paw")}<span class="ability-label">ABILITY</span></button></div><div class="beta-action-cluster" aria-label="Field actions"><button type="button" class="beta-field-tool beta-direct-action" aria-label="Hold Attack or Field Tool">${icon("axe")}<span>ATTACK</span></button><button type="button" class="beta-dodge beta-direct-action" aria-label="Dodge"><span class="beta-dodge-glyph" aria-hidden="true">↗</span><span>DODGE</span></button><button type="button" class="beta-jump beta-direct-action" aria-label="Jump"><svg viewBox="0 0 32 32" aria-hidden="true"><path d="M7 14 16 5l9 9M16 6v17M5 27h22" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/></svg><span>JUMP</span></button></div>`;
  ...
  const fullscreen = createFullscreenControl(hud, toast);
  let fieldGuideKey = '';
  ...
  const portraitEquippedButton = hud.querySelector('.portrait-equipped');
  let fieldToolPointerId = null;
  let portraitToolsOpen = false;
  ...
  fieldToolButton.addEventListener("pointerdown", (event) => {
    if (contextualVisible || fieldToolButton.disabled) return;
  ...
  const onResize = () => { if (portraitToolsOpen) { endFieldTool(); setPortraitToolsOpen(false); } };
  ...
  function setPortraitToolsOpen(value) { portraitToolsOpen = !!value; root.classList.toggle('portrait-tools-open', portraitToolsOpen); portraitEquippedButton.setAttribute('aria-expanded', String(portraitToolsOpen)); }
  function open(tab = "journal") { if (!canOpen()) return false; if (fieldToolPointerId !== null) suppressOpeningClick = true; endFieldTool(); setPortraitToolsOpen(false); if((tab === 'inventory' || tab === 'equipment') && openInventory){close();return openInventory();} welcome = false; opened = true; activeTab = tab; if (tab === "inventory") inventoryTab = "equipment"; modal.hidden = false; root.classList.add("is-open"); setBlocking(true); signature = ""; render(true); focusModal(); return true; }
  function openBuildCatalog() { workshopTab = "build"; return open("workshop"); }
  function close() { endFieldTool(); setPortraitToolsOpen(false); if (modal.contains(document.activeElement)) document.activeElement.blur(); pendingImport = null; welcome = false; opened = false; modal.hidden = true; root.classList.remove("is-open", "is-welcome"); setBlocking(false); }
  function isOpen() { return opened || welcome; }
  function showWelcome() { endFieldTool(); setPortraitToolsOpen(false); welcome = true; opened = false; modal.hidden = false; root.classList.add("is-open", "is-welcome"); setBlocking(true); render(true); focusModal(); }
  function toast(title, detail = "") { toastEl.innerHTML = `<strong>${esc(title)}</strong><span>${esc(detail)}</span>`; toastEl.hidden = false; toastEl.classList.add("show"); clearTimeout(toastTimer); toastTimer = setTimeout(() => { toastEl.classList.remove("show"); toastEl.hidden = true; }, Math.max(4200, Math.min(9000, (title.length + detail.length) * 55))); }
  ...
  function body(model) {
    ...
    const tabs = [["journal", "MAP", "map"], ["equipment", "GEAR", "backpack"], ["workshop", "WORK", "axe"], ["skills", "SKILLS", "skills"], ["wildkin", "WILDKIN", "paw"], ["settings", "SETTINGS", "settings"], ["more", "MORE", "more"]].map(([id, name, iconId]) => `<button class="tab-${id} ${activeTab === id ? "active" : ""}" data-tab="${id}">${icon(iconId, { size: 25 })}<span>${name}</span></button>`).join("");
    const panel = activeTab === "equipment" ? inventoryPanel(model) : activeTab === "workshop" ? workshop(model) : activeTab === "skills" ? createSkillTree({ skills: model.skills ?? {}, iconMarkup: icon, selectedId: selectedSkill }) : activeTab === "wildkin" ? companionPanel(model) : activeTab === "settings" ? settings(model) : activeTab === "more" ? morePanel() : journal(model);
  ...
  function updateEquipmentHud(model) {
    ...
    if (nextKey !== equipmentHudKey) {
      ...
      const count = item ? getEquipmentCount(item.id, state) : 0;
      portraitEquippedButton.innerHTML = `${equipmentIcon(item?.id, 42)}${item && !['tool','building'].includes(item.kind) ? `<b class="portrait-equipped-count">${count}</b>` : ''}`;
      portraitEquippedButton.setAttribute('aria-label', `Change equipped item: ${item?.name ?? 'Empty slot'}`);
  ...
  function update() {
    ...
    const model = getModel?.(), attempt = model?.fieldTaming;
    const observation = !attempt && !model?.observation?.complete ? model?.observation : null;
    const guide = attempt ? {label:attempt.label,detail:attempt.detail} : observation ? {label:`${observation.name} · Observe ${observation.stage}/2`,detail:observation.reason || 'Stay quiet and keep it in view.'} : null;
    const amount = observation ? Math.round(observation.progress*100)/100 : 0;
    const key = guide ? `${!!attempt}|${guide.label}|${guide.detail}|${amount}` : '';
    if (key === fieldGuideKey) return;
    fieldGuideKey = key;
    fieldGuide.hidden = !guide;
    root.classList.toggle('has-field-taming', Boolean(guide));
    fieldGuide.classList.toggle('is-observing', Boolean(observation));
    if (guide) {
      fieldGuide.querySelector('strong').textContent = guide.label;
      fieldGuide.querySelector('span').textContent = guide.detail;
      const bar=fieldGuide.querySelector('progress'),button=fieldGuide.querySelector('button');
      bar.hidden=!observation;bar.value=amount;
      button.dataset.action=attempt?'cancelTaming':'open';
      if(attempt)delete button.dataset.tab;else button.dataset.tab='wildkin';
      button.setAttribute('aria-label',attempt?'Stop taming attempt':'Read Wildkin field notes');
