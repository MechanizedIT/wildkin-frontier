...
export function createBetaShell({ app, getModel, onAction = () => null, onBlockingChanged = () => {}, canOpen = () => true, openInventory = null } = {}) {
  ...
  function open(tab = "journal") { if (!canOpen()) return false; if (fieldToolPointerId !== null) suppressOpeningClick = true; endFieldTool(); if(tab === 'inventory' && openInventory){close();return openInventory();} welcome = false; opened = true; activeTab = tab; if (tab === "inventory") inventoryTab = "equipment"; modal.hidden = false; root.classList.add("is-open"); setBlocking(true); signature = ""; render(true); focusModal(); return true; }
