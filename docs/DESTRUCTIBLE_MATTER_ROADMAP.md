...
**Status:** Phase 0.5C.1 HOLD after source-composition/seam proof and owner playtest; complete C.1 remediation before Phase 0.5D
...
**Owner playtest disposition:** camera-relative WASD and long-range visible-surface mining were subsequently added and verified. The remaining C.1 gate now includes four observed runtime/perceptual defects: (1) a mined rock can become visibly disconnected while remaining one physics actor/proxy, (2) dirt that loses support after rock extraction can remain floating because post-transfer dirt support is not yet re-resolved, (3) subsequent world dirt edits can visibly perturb a settled rock actor, and (4) triangle-majority crisp seams produce an unnatural sawtooth material boundary. The buried-rock proof must also move from a limited preview into the ordinary interactive/persisted edit path. Resolve these without returning to exact collider research before Phase 0.5D.

**Recommended C.1 completion direction:** classify every disconnected actor component immediately into retained actor / transient shard / debris even when no major brittle fracture fires; rerun local dirt support after cross-material extraction; avoid rebuilding unchanged dynamic actors during unrelated world edits; replace triangle-majority seam coloring with a shared-surface continuous material-boundary signal rendered with a hard/very narrow threshold; and exercise the buried fixture through the normal camera, mining, ownership, detachment and reload chain. Reproduce the Rapier rotation discrepancy and fix the invariant/test from evidence rather than merely lowering a threshold.
