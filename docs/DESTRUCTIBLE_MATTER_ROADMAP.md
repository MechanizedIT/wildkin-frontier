...
**Disposition (2026-09-24): HOLD.** The isolated `cellular-terrain` experiment proves a fixed 3×3 X/Z terrain patch (16³ cells/chunk; 0.5 m samples), global integer-addressed procedural composition, read-only one-sample halos, local sparse edits and derived dirty chunk sets. Independent chunk Surface Nets form a closed combined surface in the covered seam tests. Candidate render/static collision products are prepared before a synchronous installation attempt after persistence; mesh, collider, save, ownership and stale-revision failures retain the old in-memory chunk set. The install-failure test throws before product mutation and restores the previous saved revision; recovery from a partial Rapier installation is unproved. Browser interior and cross-X edits rebuild 1 and 2 chunks, reusing 8 and 7 products respectively. Four-chunk corner dependency is a focused deterministic test, not a human/browser edit. Literal sparse-edit reload is tested, but the browser receipt's stats reset after reload. Most importantly, this lab does not integrate C.1R `MatterActor`, exact ownership, bounded cross-chunk support/extraction, actor product reuse or actor persistence; the Rapier marker is not matter. No detached embedded-rock, old-location-air, actor collision, tunnel-through-seams, exact ledger, or full evidence-board proof is admitted. See [report and evidence](VOXEL_PHASE05D_REPORT.md) and `evidence/voxel-phase05d/`. Complete those D gates and repeat independent review before considering D PASS or Phase 0.5E.
