...
- September12 world descriptor: `frontierWorld` owns the default edition/seed and domain-derived salts. `frontierProgress.getWorldDescriptor` returns an immutable identity from strictly validated atlas/ecology metadata. Main loads it before terrain creation; terrain and child ecology/wildlife/scenery runtimes capture it for their lifetime. All current random streams and fallback ground/clearance samples consume it. Default output remains exact; alternate worlds are isolated generator tests, while saves reject foreign seeds until namespaced histories are implemented. Import still requires runtime reconstruction. `SIMULATION_PLAN.md` records component/process-clock proposals without adopting an ECS or changing active-play-only timing.
