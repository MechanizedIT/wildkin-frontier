...
- `gameCameraOrbit` owns empty-right-side drag, two-touch pinch and wheel input; `cameraFollow` alone owns bounded zoom/yaw and fixed pitch. A left joystick or UI touch never joins a pinch. Input capture resets on cancel, blur, hidden state and viewport resize. `fullscreenControl` requests browser fullscreen only from a click and follows actual browser state, with an unavailable/refused fallback. The existing loop renders both viewport orientations.
