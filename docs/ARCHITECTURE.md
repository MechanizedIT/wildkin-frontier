...
- September13 Signal Cache: `frontierDiscovery` validates one immutable fixed-authored site against canonical terrain, both object footprints, adjacent life/scenery, admitted model paths and loot table. `lootRegistryOverlay` registers its identity before save loading without changing authored data; section interaction lists include it only while physically resident. `frontierDiscoveryRuntime` owns one receiver/chest pair and two simple solid collider meshes, explicit model disposal, cache registration and occlusion invalidation. Main establishes these colliders before movement/resume queries. `cacheMechanisms` supports explicit register/unregister with claimed-lid restoration; `lootSystem`/`frontierProgress` retain reward, partial remainder and save authority. Alternate seeds and distributed secrets remain later work.
...
- September13 regional-place checkpoint: `frontierRegionalPlace` owns one pure crystal-bloom geometry recipe and the shared catalog-admission predicate used by ecology, scenery and wildlife. A seeded4×4 macrocell proposes one raw owner; local priority thinning leaves at most one owner in any3×3 chunk window without depending on load order or recursively sampling neighboring sites. Admission requires95% reserve fade, at least65% Sunscar province weight, complete7m support at slope0.28 or below, and a107m buffer from protected Camp/starter scenery. One real finite crystal uses persistent resource slot200; the cleft composition may add separated slot201. Slots202–207 remain reserved. Seven admitted decor specs enter scenery as one group. Neighboring forage checks the final scaled candidate footprint, scenery/grass reserve the whole place, and wildlife excludes its complete home movement disk. `frontierWildlifeRuntime` passes the admitted visual-asset catalog into the pure wildlife recipe. The fan and cleft layouts provide bounded early variation, not an endlessly unique-place guarantee. Selected R3 scored 8/10; current aggregate verification covers1,243 tests. Native/portable evidence and limits are retained at `art/reviews/regional-blooms/receipt.md`.
