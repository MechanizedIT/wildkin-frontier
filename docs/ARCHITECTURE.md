...
- September13 first-outing purpose: `frontierPurpose` is a pure derived view over saved progress, carried cargo, Camp-reachable spendable resources, health and the selected companion ability. It owns no quest state, rewards or inventory. `createBetaGame` supplies authoritative inputs and retains action ownership; `betaShell` renders one compact clickable plan, suppresses it during observation/taming and highlights only a ready active Mossling Bloom. Build/craft affordability may include the selected reachable storage, while physical feed/plant actions still require a backpack berry. Successful pack-changing progress commits notify `main`, which refreshes the existing resource HUD and expedition cargo; failed saves and storage-only changes emit no carried-resource update. Native proof reaches settled/fully fed care, a planted adjacent garden, its actual four-berry harvest, and an isolated Bloom heal/cooldown check. Developer and packaged literal reload parity closes the gameplay/persistence checkpoint. Garden construction required temporary landscape framing in that historical journey; the following portrait construction batch resolves the workaround. Evidence: `art/reviews/alpha-outing/receipt.md`.
