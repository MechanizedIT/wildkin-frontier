...
- Larger continent checkpoint: `world/frontierContinent.js` owns a validated 47-vertex authored outline, fixed 6 × 7 km bounds and small seed variation, rounded-unioned with the old land field. Polygon queries use exact nearest edges with private bounded spatial candidates; at most four samplers are cached. Existing terrain/shelf/content/water/current/resume consumers share its published fields. `frontierRegionalPlace.js` owns fixed grove catalog bounds (4,368 chunks); `frontierDiscovery.js` imports the same bounds before save normalization. The current catalog is 85 groves plus Signal; all original IDs remain. `tools/inspect-frontier.mjs --overview` returns a bounded terrain/region-only PNG/SVG/JSON and exits before ecology imports. Proof: frontierContinent, frontierDiscovery, frontierRegionalPlace, lushGroveLootLifecycle, frontierInspectorTool and coast sibling tests. [Current receipt](../art/reviews/continent-v2/receipt.md).
