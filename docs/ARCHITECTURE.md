...
- September13 scenery preparation: `createFrontierSceneryBuild` shares one ephemeral bounded9×9 source cache between selection and ground-cover clearance. Representative duplicate recipe calls drop666→98 with exact output parity; measured native scenery bursts608.2/455.5ms become191.3/146.5ms. The cache owns no loop or durable state. Rejected visual construction is disposed on a physics replacement error; remaining nontransactional physics-batch and terrain/ecology failure hazards are documented in `art/reviews/frontier-discoveries/streaming-audit.md`.
