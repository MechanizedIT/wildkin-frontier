...
> **Active architectural direction — September 25:** the browser voxel R&D through Phase 0.5E has completed its bounded proof and is now the executable behavioral reference. The production target is a higher-detail **PC-first native game**, with **Unity 6.3 LTS + HDRP** as the first engine candidate. Native qualification will test Codex/Unity agent automation, a custom sparse matter kernel, Surface Nets vs Dual Contouring, uniform vs locally refined bricks, projected stylized materials, procedural SDF stamps, and a minimal destruction slice before a production migration is approved. Unreal 5.8 remains a deferred challenger if Unity exposes a material blocker. See [UNITY_FIRST_TRANSITION_PLAN.md](UNITY_FIRST_TRANSITION_PLAN.md), [PC_ENGINE_BAKEOFF_PLAN.md](PC_ENGINE_BAKEOFF_PLAN.md), and [NATIVE_DESTRUCTION_REPRESENTATION.md](NATIVE_DESTRUCTION_REPRESENTATION.md).
>
> **September 21 direction — historical R&D foundation:** the finite heightfield/static-world direction was superseded by the cubic-chunk destructible-matter experiments. Preserve those verified matter contracts and negative evidence, but Three.js/Rapier/browser packaging are no longer production constraints.
...
<!-- SCOUT-CONTRACT -->
**Scout inspection contract (September13):** an explicit `?scout=1` session clones ordinary progress lazily into a page-local storage adapter injected into the existing progress owner. All progress reads/writes use that adapter; normal/Author defaults are unchanged and Author takes precedence if both queries appear. Exit/reload discards Scout progress. The existing character body remains the streaming center. A pure displacement helper skips ordinary gravity/collision while flying and clamps above sampled terrain/water; toggling off returns through ordinary falling/landing. The normal loop, camera, scene and wildlife continue. Shared combat protection and the fatal-flow guard protect the whole Scout session. Space/C/Shift reuse keyboard ownership, G/pointer lift belong to the small Scout UI, and modal blocking clears movement. No new RAF, persistent mode, save schema, network request or dependency. [Validation](../art/reviews/continent-restart/scout-receipt.md).
<!-- END SCOUT-CONTRACT -->

Heartwood circuit retains the current scenery pipeline: immutable center-independent recipes append 28 fixed circuit pieces, and selection explicitly owns their protected starter-only visibility and budget. The STAGED table is unchanged because it also controls infill protection. Special surfaces/coast take precedence; neighboring modes/infill filter the new pieces before admission. A private exact-accent option admits only three circuit cloudflowers. No terrain, save, visual-runtime or frame-loop owner changed. Shared cache reversal, all center caps and prior gameplay IDs pass; native prepared publication and seven enabled non-climbable trunk surfaces pass. [Contract](HEARTWOOD_CIRCUIT_CONTRACT.md).

Heartwood checkpoint: a late prefix-only authoring composer adds the twenty static Camp props through existing registry/render/collision owners; no runtime scenery, terrain, state or save owner is added. Existing purpose derivation presents builder and collector choices without new quest/preferences. The local HTTP server closes abandoned response streams through Node pipeline; real-byte and aborted-file-release proof passes. [Contract](HEARTWOOD_OPENING_CONTRACT.md).
...
- September13 Tidal Ward: `playerCombat` alone owns transient `wardRemaining`; max-preserving `grantWard` caps at5s, and Tidefin requests3s. Fixed-step expiry, reset/restore and `isInvulnerable` share this owner while retaining independent post-hit/dodge grace. Companion ability data adds active remaining/duration separately from its existing26s cooldown; other species expose zero active time. The existing HUD and dome consume this state, including paused/hidden presentation, without a new buff framework or loop. Failed seals grant nothing; routine Ward success adds no covering toast. Both territorial rusher/spitter WARN exits now use the same `warnTime` threshold as target eligibility, fixing a one-tick reset cycle without changing awareness or retreat rules. All current guarded damage paths and ability siblings were reviewed. Ward and every ability cooldown remain unsaved and ready after reload; this pre-alpha policy is explicit debt. Native and portable evidence: `art/reviews/tidal-ward/receipt.md`.

- September13 Cragbreaker mining: pure `resources/mineralStrike.js` selects at most three READY active stone/iron/crystal nodes within4.5m in3D, sorted by distance then lexical ID, and requests at most four normal hits each. `main.applyHarvestHit` shares Camp/Rootfall before guards, existing resource save-before-effect, pickup/bonus and after guards between field tool and ability. `resourceSystem` retains sole depletion, collision and regrowth ownership. `companionSystem` retains selection, combat, seal and cooldown; it invokes the injected strike only for Emberhorn. First rejection stops the burst without rolling back earlier committed hits, consumes the usual cooldown and preserves the resource owner's failure toast. Coalesced particles/sound run only on the last intended hit per source. No new schema, transaction owner, scheduler or dependency. Authored/regrowing and generated/finite sibling paths are covered; receipt: `art/reviews/cragbreaker-mining/receipt.md`.

- September13 portrait construction: `basePlacement.findInitialPlacement` tries at most40 unique nearby positions through the unchanged footprint/support/reservation validator. `baseSystem` owns the selected target, retryable preview and exactly-once begin/end lifecycle. `camera/constructionView` holds only a temporary restoration snapshot; the existing `cameraFollow` remains authoritative. Portrait open frames the target at42°/zoom1, Place retains its yaw and restores pitch/zoom, and Cancel/external close restores all three. Capture-phase viewport listeners close before the next camera profile sync, preserving independent orientation pitches. Failed saves keep framing for retry; no camera/save framework or loop is added. All11 piece families share this path. Proof: `art/reviews/portrait-construction/receipt.md`.

- September13 first-outing purpose: `frontierPurpose` is a pure derived view over saved progress, carried cargo, Camp-reachable spendable resources, health and the selected companion ability. It owns no quest state, rewards, inventory or ledger. Pending bonds, useful ready Bloom healing and urgent low health retain priority. Once the active Mossling care loop has a bed and garden, away play derives `seek-rootbound-grove` instead of a crop/young return chore; Camp still derives actionable care, garden and nursery priorities. `createBetaGame` supplies authoritative inputs and retains action ownership; `betaShell` renders one compact clickable plan, suppresses it during observation/taming and highlights only a ready active Mossling Bloom. Build/craft affordability may include the selected reachable storage, while physical feed/plant actions still require a backpack berry. Successful pack-changing progress commits notify `main`, which refreshes the existing resource HUD and expedition cargo; failed saves and storage-only changes emit no carried-resource update. Focused proof and independent review pass while the earned journey continues. Evidence: `art/reviews/alpha-outing/receipt.md`.
- September13 finite discoveries: `frontierDiscovery` builds one canonical catalog of the fixed Signal Cache and 85 current Lush grove chests before `frontierProgress.load`, so normalization sees every stable identity before residency. Signal admission still checks its receiver/chest terrain, assets and adjacent content; grove admission consumes only admitted `lush-root-cache` places and their supported chest anchors. `lootRegistryOverlay` indexes the complete catalog and exposes generated interactions only for resident IDs. `frontierDiscoveryRuntime` indexes definitions by chunk and owns bounded multi-residency, staged visual/collider publication, explicit mechanism/occlusion retirement and `.7` grove chest visual/collider parity. Main establishes resident colliders before movement/resume queries. `lootSystem` and `frontierProgress` remain the sole atomic one-time claim, partial remainder, XP and rollback owners. `lushGroveLootLifecycle.test.js` covers independent seals/remainders across unload/reload and failed writes.
- September13 terrain preparation: `frontierChunkRuntime` retains the current 5×5 published neighborhood while the authoritative tick may build one detached incoming chunk per update near an anticipated adjacent boundary. It exposes one immutable anticipated-residency descriptor whose membership exactly matches the eventual full window. Preparation is capped at nine chunks (five for an axis crossing); zero-motion render frames retain the same descriptor only while the player remains inside its signed preload corridor. Nonzero direction changes, leaving that corridor, inactive/Author mode and disposal retire speculative ownership. Detached roots have no live colliders. A complete replacement window is prepared before physics installation and visual publication; failed construction/installation disposes candidates and preserves the previous snapshot for retry. Initial load, resume and teleport remain synchronous. No new loop or general scheduler is added.
- Shared streamed physics installation stages all incoming colliders disabled before removing existing ones. An allocation/descriptor failure removes the staged set and leaves existing support and indexes intact. Successful publication preserves terrain/rock traversal, ordinary scenery/discovery solids, section activation and disabled-object state, then steps Rapier once. This is a tested allocation-failure boundary, not a claim of rollback after an unrecoverable engine removal/enable/step failure. Ecology's higher-level lifecycle is unchanged.
- Scenery preparation retains overlapping pure forage/wildlife and ordinary/infill recipes in the existing bounded cache. `frontierSceneryRuntime` owns at most one target: one resumable recipe job followed by one detached visual job. The existing RAF advances that work under a soft3ms/maximum256-work-unit budget. Matching early arrival drains the same unfinished job; cold load and teleport use the synchronous builder. Reversal, inactive residency, failure and disposal cancel job-local partial recipes and dispose detached GPU resources, while complete immutable recipes may remain inside the fixed cache bounds.
- Synchronous and incremental visuals share one builder and exact ordering, IDs, transforms, colors and surfaces. A fixed-world ground-patch cache retains at most512 complete patches of at most28 records each before the global640-cluster cut; it owns no Three.js resources and is pruned to current/speculative IDs. Exact full-window matching is required before transfer. Scenery colliders install successfully before the old scene is retired; failed physics disposes only the candidate and leaves the old resident retryable. Exact-coordinate height/full-sample memos remain separate8,192-entry build-local maps and clear after completion or failure. One aggregate passed1,350 tests in77.391s. Native boundary B reduced scenery270.5→16.6ms and the maximum frame420.6→161ms; preparation reached15.5ms, so the3ms budget is explicitly soft. Ecology86.6ms and terrain preparation20–27ms remain measured work. No new visual round was needed; the prior6.8/10 HOLD remains. Evidence: `art/reviews/scenery-preload/receipt.md`.

- September13 provisional coast/swim foundation: `frontierContinent` owns one finite seeded continent centered at(-700,-900), sea level-2, signed coast distance and inward direction. `frontierTerrain` preserves inland terrain beyond a90m dry transition, continues a12m shallow bed and32m shelf, and indexes a triangle only when all three vertices remain inside that shelf. Deep-ocean residents therefore publish no ground mesh, grass or Rapier trimesh. `frontierOceanRuntime` borrows the immutable terrain residency snapshot and caps presentation at three draws; it adds no RAF or world state. `surfaceSwim` owns water normalization, WADE/SWIM thresholds, buoyancy and the visible inward current; current force joins desired swim velocity before normal acceleration. `playerController` remains the KCC/mode owner. SWIM blocks field tool, combat, jump, dodge, taming and companion abilities; WADE retains ordinary actions. `companionSystem` waits followers at their last dry point through WADE/SWIM. `expeditionPersistence` preserves same-run last-dry feet with live health/XP/pending changes, while `resumePosition` still requires ordinary Rapier support for original, coast-adjusted or Camp fallback candidates. This is one early-alpha continent with no boats, diving, aquatic roster or drowning system. Independent behavior review and all1,300 tests/package gates pass; selected R3 visuals remain6.4/10 HOLD. The current mesh also reserves18 wake ribbons (180 combined current/wake vertices), retaining the three-draw ceiling. Evidence: `art/reviews/frontier-coast/receipt.md`.

- Historical September12 province foundation (site allocation superseded above): `frontierRegion` owns one pure deterministic field of jittered roughly600m sites, compact smooth weights and three terrain targets clamped to0..84m. `frontierTerrain` blends that target outside the exact Camp/apron, starter-route and Skybreak reserve, then publishes `provinceId`, `provinceKind`, `provinceInfluence` and `provinceWeights` separately from existing Camp/section metadata. Ecology, scenery and wildlife consume the same sample using admitted assets and stable generated IDs; existing resident/count caps remain authoritative, including at most one additional high-draw regional signature wildlife resident. It added no save schema, loop, dependency, weather or overhang contract; the later coast foundation above now bounds the field with water.

- September13 regional places: `frontierRegionalPlace` keeps one deterministic4×4 owner field, local priority thinning and shared reserve/dry/full7m support gates, then applies family-specific asset and composition admission. Sunscar retains its fan/cleft crystal layouts and stable resource slots200–201. Lush admits only the exact canopy/log/stone/ruin kit, seven whole-group scenery slots, an open supported approach and the `.7` chest anchor; stateful Rootfall is excluded. Forage, scenery, grass, wildlife homes and sibling places consume the same final family footprint, preserving existing Sunscar outputs. The original grove checkpoint contained 59 Lush groves; the coast expansion now admits 85 within the same finite catalog bounds. Selected grove R3 is7.7/10 HOLD; all1,319 tests and verify/world/campaign/build/validate/ZIP pass. Native full-health opening/collection, two-site Rapier/residency and complete developer/package continuation also pass. Evidence: `art/reviews/lush-groves/receipt.md`.

- Province presentation preserves the terrain/physics contract. `frontierTextureColor` samples a globally aligned2m color grid, bounded to27×27 points per chunk, for baked texture color only. Analytical vertex samples still supply the mesh and matching Rapier triangles; the regular2m grid retains its small between-vertex query approximation, while Skybreak keeps exact1m triangle queries. Reserve/fade texture chunks use exact colors. The atlas retains its shared sampler and now shades taller elevations. Evidence: `art/reviews/regional-provinces/receipt.md`.

- Skybreak ecology shares the existing sample and resident owners. `surfaceKind` labels cap, shoulder and true lowland surfaces; open ground inside the rectangle stays unlabelled. Reserved source indices100+ keep ordinary forage/wildlife identities intact. Actual asset metadata supplies scaled support footprints and neighboring forage/grass exclusions. The crown Mossling has a locally bounded home whose complete movement disk is checked in half-metre steps; a flat center/outer ring alone does not establish a traversable interior. Only soft cap cloudflowers receive the precise home-clearance exception. No new frame loop, save authority, runtime dependency or terrain geometry is introduced.

- Resume and checkpoint closure: `resumePosition` validates the capsule sole above a legal incline using the ray normal and capsule curvature, without moving the player probe or bypassing slope/overlap/kill checks. `expeditionPersistence` may reuse only supported feet from the same run and section while rebuilding health, XP, pending bonds, Core and discoveries from live owners. A rejected position must not roll back current fall damage or rewards. Focused real-Rapier and persistence regressions cover these sibling paths.

- Skybreak terrain contract: `frontierLandform` owns one bounded plateau recipe and its route/anchors; `frontierTerrain` uses 1 m detail in four chunks only. Its public support query interpolates the exact float32 mesh triangle; detailed/coarse boundaries use the actual neighboring edge curve, including the older terrace breakpoint. `frontierChunkRuntime` shares that geometry with Rapier, applies regional cliff materials and footprint-safe grass, and retains bounded residency. `frontierPlacement.hasFootprintSupport` supplies center/ring slope checks to forage, scenery and wildlife. Ordinary roaming receives a full home footprint; this is not a navigation guarantee for fleeing or pursuit. One height remains available per XZ point; overhangs/cave surfaces are future work. See CURRENT_SLICE and the Skybreak receipt for final admission/validation.

- September12 world descriptor: `frontierWorld` owns the default edition/seed, normalization and domain-derived salts. `frontierProgress.getWorldDescriptor` returns an immutable identity from strictly validated atlas/ecology metadata. Main loads it before terrain creation; terrain and child ecology/wildlife/scenery runtimes capture it for their lifetime. All current random streams and fallback ground/clearance samples consume it. Default output remains exact; alternate worlds are isolated generator tests, while saves reject foreign seeds until namespaced histories are implemented. Import still requires runtime reconstruction. `tools/inspect-frontier.mjs` reads this contract into bounded SVG/JSON diagnostics without mutating saves; its wetland view remains current habitat weight rather than climate moisture. The active province field is the first regional runtime consumer; later climate, drainage and three additional regional families remain provisional.

- September12 shared process timing: `src/game/savedProcessClock.js` owns only a transient active-play accumulator. `campGarden` and `campBreedingGrowth` inject their own saved process readers and atomic advance transactions, retaining separate crop/offspring rules and save authority. Progress commits in five-second quanta and remains stopped while paused, hidden, offline or in Author. The helper adds no durable schema, offline catch-up, ECS, loop or claimed measured speedup; future machine, weather and day/night clocks remain unimplemented.
...
- September12 F2D (historical geometry checkpoint, superseded by F2F grass): scenery residency/merged geometry permitted72 tufts. SelectedR2 retained26 inspected props/15 compact colliders/one low draw19954triangles. It remained below the visual target. The world descriptor entry above supersedes its earlier incomplete seed wiring.
- September12 F1B/F4B: `frontierTerrain` samples one local signed rolling relief for height and presentation color, with exact Camp/terrace guards and shared mesh/physics/atlas grounding. The color cue does not alter habitat classification. `wildkinAppearance` strengthens the existing whole-body tint to.78 across wild/follower/young instances; no geometry, save schema or additional loop. Terrain art remains HOLD; body-tone comparison passes provisionally.
...
- September12 F5B: `climbProbe` owns bounded natural-face discovery and freezes the eligible terrain/rock collider handle, face contact/normal, top and clear exit; streamed `traversalSurface` tags admit only `terrain`/`rock` and exclude scenery. `climbingController` owns attach, slow up/down/hold, drop, face retirement, blocked-exit fall and collision-resolved mantle through the existing player KCC. Portrait exposes the same contextual climb/drop action. KCC movement and mantle exits remain clearance checked; central slope-aware sole support is the shared grounded authority for movement, jumps, falls and knockback. Player fall tracking closes ordinary, climb-detach and knockback landings once. The visual adapter receives staged Climb/Mantle/Fall phase metadata, but palm/boot contact and mantle transfer remain an art hold.

- September12 F2C: `frontierScenery` owns pure seeded placement/selection; `frontierSceneryRuntime` borrows the immutable terrain residency snapshot and owns one bounded scene/surface/fade lifecycle. `frontierSceneryVisual` retains shared canopy templates and owns merged low-plant/tuft geometry plus compact outward-wound solid surfaces. No new save state, terrain-height owner or loop. Update after terrain residency and before movement/resume queries; unregister fade and remove collider batches before retiring visuals. Aggregate scenery participates in existing contextual LOS invalidation. Receipt: `art/reviews/living-frontier-f2c/receipt.md`.

- September12 M1: portrait is primary. `cameraFollow` owns a stable42°/1.2 portrait profile and separate orientation pitch preferences; `touchMovement` owns a fixed portrait stick and both viewport cancellation paths. `betaShell` owns compact tool-belt presentation and cancellation of held tool input. `contextualInteraction` owns thumb/world-status presentation and exact-action gesture tokens; nearby gameplay owners retain physical range and transaction validation. `baseSystem` scores nearer visible furniture ahead of the yard console, while physical storage and station targets retain priority; reach checks, action dispatch and save ownership are unchanged. Focused proof and independent review pass; root's native garden-overlap check exercised HARVEST 4 followed by Plant. `worldInteractionAnchor` provides bounded eight-target geometry/LOS caches with10Hz reuse, explicit invalidation, and visible-candidate predicates. `styles/portrait.css` owns the new layout without replacing the landscape controls. Native/package evidence: `art/reviews/living-frontier-m1/receipt.md`.
...
- `inventory/itemCatalog`, `slotOperations`, `inventoryState` and `inventoryActions` define item limits, pure stack transactions, saved container shape and an injected access/save adapter. `physicalInventory` owns proximity, one chosen crafting source and panel coordination. `inventoryPanel` owns only transient focus/selection/drag/split state. Pack contents survive travel and extraction; storage is accessible only near its physical Camp object. Container construction/removal, costs, output fitting and refunds commit through the progress owner.
- `expeditionPersistence` snapshots through the existing frame loop every three seconds and on pause/background boundaries. `activeRunState` validates the compact record; `resumePosition` verifies actual Rapier support, clearance and hazards before restoration, falling back to safe same-region entries. Inventory/reward transactions capture post-operation XP, health and pending bonds in the same save. Failed death resolution preserves the old record, blocks play and retries; successful resolution clears it atomically. Ordinary world actors rebuild after reload; unfinished taming attempts are not resumed.
...
- `companionSystem` owns transient newly bonded IDs, bonding target, cooldown and bounded visual followers. `bondingLogic` is rendering-free. Capture removes wildlife without granting combat XP. Companion seal access resolves explicit `requiredCompanionId` with the legacy fixed-secret map as fallback; an ability selects the nearest unopened matching resident in the active section, breaking equal distances by stable ID. `completePoi` must commit before any ability effect or cooldown. Marked `opensOnSeal` chest lids follow saved completion through `cacheMechanisms` and block collection while opening; ordinary claim-driven lids and legacy vaults keep their existing behavior. `companionSealMechanics.test.js` covers matching, ties, multiple sites, healthy Bloom and retryable failed writes. Successful extraction banks pending bonds with cargo/XP/Core as one transaction; death clears them.
- September 11: `fieldTaming` owns one transient species attempt and passes movement intent into the existing CreatureSystem/Rapier path. `companionFollowIntent` decides loose follow/settle/interest goals; `companionPhysics` owns each follower's kinematic controller/gravity. Player, wild and follower filters keep companions from obstructing each other while retaining world collision. All use the one fixed/frame loop.
- September 11 evening: `creatureObservation` owns one transient quiet study, using the existing creature awareness, active region, visible viewport and actual player/camera sight rays. `observationCatalog` owns species timing and Journal prose. `frontierProgress` saves only bounded completed clue stages in the optional v3 `observationClues` field; acquisition/discovery commits atomically and failed writes retry. Basic taming remains available without observation. `sightRay` supplies one correct raised-origin ray contract to observation and creature attacks.
- `baseCatalog` owns shared pieces/costs and field recipes; `basePlacement` validates footprints/support/reserved Camp space. `frontierProgress` owns persistent base tiers/structures/supplies and atomic costs/refunds. `baseSystem` owns placement input, preview, instances and Camp-only colliders, with explicit Author suppression. `basePieceVisual` supplies the same fitted model to placement and baked catalog thumbnails. These player structures are separate from the editable authored world.
- `lootSystem` owns presentation and transient Core possession. `frontierProgress.claimLootRewards` commits accepted stacks, partial remainders, one-time XP and current Core possession together. Core material/XP rewards occur once; its mission possession can be recovered after loss until successfully extracted. Capacity rejection preserves unaccepted rewards. Harvest pickups aggregate the finite current harvest cycle per source; uncollected drops never block further hits. Only depleted-node regrowth waits for its outstanding drop to clear, bounding field overflow without stalling harvesting. Accepted stacks belong to saved inventory; uncollected ground rewards remain transient with ordinary rebuilt world actors.
- `gameCameraOrbit` owns empty-right-side drag, two-touch pinch and wheel input; `cameraFollow` alone owns bounded requested zoom/yaw/pitch and temporary collision distance. `cameraCollision` queries the explicit camera-solid registry; authored terrain and solid props plus runtime construction/defenses participate, while three canopy families keep fading. Focus recovery validates current terrain height and a clear corridor from player focus; zero-distance look direction preserves movement heading. A left joystick or UI touch never joins a pinch. Input capture resets on cancel, blur, hidden state and viewport resize. `fullscreenControl` requests browser fullscreen only from a click and follows actual browser state, with an unavailable/refused fallback. The existing loop renders both viewport orientations.
- `contextualInteraction` projects a live target's cached body envelope each frame and retains its prior valid side while updating position. HUD obstacles are sampled at10Hz; preferred placement must still pass body/control/bounds checks. `inventoryPanel` uses square cells, a lifted/clamped touch ghost and per-container scroll cues; only real pointer coordinates select a drop destination.
...
- Presentation modules own bounded terrain detail, atmosphere, contact shadows, floating combat text and companion effects. A single 1024px sun shadow pass supplies depth; prop occlusion checks upper and lower player sightlines at 10 Hz and clones only affected instance materials. Authored scenery is indexed initially; Camp construction explicitly registers and unregisters its visual roots through injected lifecycle callbacks, restoring shared materials before removal. These modules update through the existing loop, suppress editor overlays, and never decide gameplay outcomes.
- Fixed library props may carry a small explicit `convexHull` collision descriptor: 4–64 local points and a closed outward triangle surface, authored offline independently of render detail. The validator rejects non-convex, open or degenerate data and excludes harvestable/wildlife roles. Shared descriptors apply uniform scale, offset and yaw once; Rapier uses the convex surface and Author draws the same faces. Camp clearance and campaign readiness use conservative derived bounds. Existing boxes, creature capsules and terrain meshes keep their paths; no runtime hull decomposition or second physics engine is introduced. The first two component models and four placed outcrops pass actual bank/contact/Author review; see `SHATTERFEN_BANK_REVIEW.md` for evidence and limits.
- Static color-only recipe props combine compatible materials through vertex colors; textured/skinned and materially different surfaces remain separate. Tagged roots, serialized recipes and source buffers remain intact. The sun shadow volume follows the player; current 30–56 m linear biome fog preserves nearby contrast before the 60 m camera clip. Regions remain the coarse visual/physics/simulation boundary; all model templates are still loaded locally at bootstrap, not streamed from the internet.
...
September 11 additions: `frontierProgress` also owns normalized base structures/expansion, field-supply counts and five-slot loadout references. `baseSystem` owns placed visuals/colliders and the actual-model placement preview; `equipmentSystem` routes selected-use input into existing tool/healing/taming/build owners and suppresses stale holds. `fieldTaming` owns one transient in-world attempt. `worldInteractionAnchor` projects the authoritative selected interaction's actual position; `interactionPriority` resolves incidental wildlife versus deliberate active taming and anchored extraction. Companion controllers own independent kinematic movement against shared static terrain; no additional physics step or frame loop is added. `naturalBoundary` provides matching scenic boundary vertices for rendering and Rapier. Region activation and distance fog remain coarse visibility/physics boundaries; model templates are still preloaded, so this is not full streamed asset-memory unloading.
...
`campLayout` owns the versioned apron/yard union, perimeter and shared placement reservations. `frontierProgress.base.layout` saves cleared IDs and the paid expansion; legacy footprint migration happens before structure/container normalization. `campClearing` commits the final clearing decision before a normal resource hit, then masks the depleted source afterward. `resourceSystem.setRemovedResourceIds` owns hidden/collider/timer/target lifecycle; pickups retain their existing source-region and quantity authority. Cleared props stop reserving build space through the same validator used by preview, payment and reload.

`campDefenses` renders admitted local panel GLBs and matching panel/low-foot/convex-brace colliders from that footprint; suppression, expansion and disposal refresh Rapier broadphase deliberately. It supplies the actual console anchor to the shared world-label projector. Expansion activation is Camp-only and requires console proximity. The canonical composer relocates scenery outside legacy footprints and adds a material-only work pad, without grading terrain under saved structures. One root loop remains authoritative.

## Ordinary jump and contour terrain — September12 working tree

Keyboard/touch own one-shot jump edges; main consumes them only after an eligible fixed movement step, clears them for blocked input and resets on placement/Author transitions. playerController owns ordinary Jump/Fall, 0.10s buffer/0.08s coyote grace and the existing Rapier kinematic air path. Space jumps; R dodges. No separate physics engine, render loop or inventory state is added. Native landscape/portrait jump input, airborne pose and landing fixtures pass; continuous animation finesse and physical-phone acceptance remain separate perceptual gates.

worldHazardSystem replaces active parkour protection with ordinary fatal-volume enter/reset behavior. Current canonical worlds contain no course pads, checkpoints, protected zones or course hazards. Historical import schema/fixtures remain readable, but active Author creation/guidance and Play rendering of retired course objects are removed. Chest IDs and loot IDs remain stable as ordinary hidden caches; loot claims do not complete a course.

terrainSurfaceModel retains a single height owner for rendering, Rapier buffers, support queries and Author rebase/export. A graded route may give every point an elevation; the nearest segment linearly interpolates that height across its full-width path with a feather into surrounding ground. Constant-elevation routes retain their prior meaning. Mixed/missing point heights or simultaneous level override reject before any rebase. TERRAIN_LIMITS bounds terrain/route height at24m; this permits deliberate connected uplands, not unbounded terrain generation. No elevated-water or stacked-overhang system is added. Verdant V2 composition and sequential native Survey/climb/shelf jump/cache/descent pass; denser V3 is separately under review. Optional height outlines use normalized local polygon corners, bounded inward edge widths and atomic rejection of crossed/degenerate outlines. The same height query supplies rendering, Rapier buffers, grounding and Author rebase/export. The Landscape editor exposes Shelf corners and graded path points. Support-only routes may set paint:false, avoiding large painted aprons. Connected ponds draw only external shoreline segments; slope materials no longer add repeated contour bands.

Canopy family collision uses exported lower-trunk bounds: a1x2.45x1m local box atY1.225 for standard/tall/spread. Existing instance Solid flags remain authoritative, including non-solid Camp backdrop. Dense Verdant woodland explicitly opts into solid trunks; wide crowns do not become invisible walls. See tools/register-ecology-assets.mjs and canopyCollision.test.js. CURRENT_SLICE owns canonical/native integration status.
...
## September13 density foundation

Scenery remains a terrain-residency consumer with no loop or save. `frontierScenery` owns immutable ordinary/infill recipes, near-only infill, finite budgets, footprint/exclusion rules and bounded temporary memo work. `frontierFixedSites` owns Signal coordinates and scenery/regional-place reserves, consumed by discovery and both placement families. `frontierSceneryVisual` compiles unique asset geometry once per build and shares it across12.5m cell/asset instances with complete native bounds; ground/low buffers and shared assets retire exactly once on success/failure. `frontierSceneryRuntime` retains transactional visual/physics publication and bounded recipes; `frontierChunkRuntime.heightMatchesSample` is an immutable explicit guarantee enabling same-height memo reuse. Detailed terrain still uses its established exact triangle query. Synchronous recipe/ground preparation still hitches and is the next performance owner problem. See `art/reviews/scenery-density/receipt.md`.


## Caldera candidate ownership — September13

The finite region sampler blends frontierCaldera once by habitat weight; frontierTerrain publishes its feature metadata and the full/reduced height chain remains authoritative for mesh and physics. Config bowlRefuge is shared with wildlife, avoiding duplicate home geometry. Ecology owns finite slots300/301, wildlife owns the one existing Emberhorn actor, scenery owns admitted bounded recipes/complete-kit retries, and sceneryVisual keys Caldera ground-cover appearance in its existing complete-patch cache. The field plan samples only the player location through one injected callback. Main delivers an attack edge on only the first fixed substep of a render frame, preserving zero-step buffering and held cadence. See ../art/reviews/caldera/receipt.md.
