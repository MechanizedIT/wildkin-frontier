...
- September13 terrain preparation: `frontierChunkRuntime` retains the current 5×5 published neighborhood while the authoritative tick may build one detached incoming chunk per update near an anticipated adjacent boundary. Preparation is capped at nine chunks (five for an axis crossing). Detached roots have no live colliders. A complete replacement window is prepared before physics installation and visual publication; failed construction/installation disposes candidates and preserves the previous snapshot for retry. Initial load, resume and teleport remain synchronous. Direction cancellation, inactive/Author mode and disposal retire speculative ownership. No new loop or general scheduler is added.
...
- Scenery preparation retains only overlapping pure forage/wildlife recipes within a bounded expanded 9×9 window, owned by `frontierSceneryRuntime` and cleared on inactive/dispose. An axis shift generates seven newly required recipes per domain instead of rebuilding all49. The cache changes neither selected placements nor durable save state. Rejected visuals are disposed on physics installation failure. Current checkpoint evidence: `art/reviews/streaming-preparation/receipt.md`; the earlier `frontier-discoveries/streaming-audit.md` remains a historical baseline.
- Each scenery rebuild also shares exact-coordinate height and full terrain-sample memos with selection, ground-cover clearance and visual placement. The two APIs retain separate semantics and separate8,192-entry caps; excess points compute normally. Both maps clear after synchronous visual construction, including failure. Representative identical26-spec/338-cluster output uses5,692 underlying terrain lookups instead of7,861 (27.6% fewer), with exact tested instance matrices/colors. These memos do not become a growing runtime world cache.
