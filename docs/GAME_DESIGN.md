...
The playable foundation is a physical Camp and short outing loop: leave Camp, explore a bounded nearby frontier, use the personal atlas, encounter or capture individual Wildkin, return to Camp, and continue from a browser-local save. The world currently uses one fixed deterministic edition with a shared descriptor across terrain, ecology, scenery, and wildlife. The inspector exposes that current generation evidence for development only.
