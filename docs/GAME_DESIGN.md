...
The latest producer release scope is one seeded continent, with later boat access to additional continents. The owner proposed roughly10habitats and30species as a content target; the first alpha uses a smaller useful set. Each Wildkin needs a practical exploration or Camp role. Read `EARLY_ALPHA_PLAN.md` for current/near/later priorities. Coastline, ocean and the target roster are not implemented.

## Proposed systems, clearly separate from the build

The intended frontier grows through large seeded regions with distinct landform, traversal, life, resources, and discovery grammar. The plan rejects predictable global path grids and palette-only biome swaps. Separate overhang meshes/cave instances, swimming, broad climbing, weather/day-night, modular compatible Wildkin parts, additional reproduction modes, habitat/food/camp construction, DNA archives/cloning, community discoveries, and trusted online trade are proposed work. They are not current gameplay promises.
