...
**September12 development additions:** ordinary Jump and natural high-ground routes replace pads/checkpoint courses. Forest Edge has connected uplands; Shatterfen's selected100×100m wetland composition adds dry banks, eastern plates and a northwest wreck-cache loop while keeping its existing actors/resources/anchors. The Backpack arranges five persistent item shortcuts without moving supplies. Rootfall's cut/brace passage and one defended Camp expansion are implemented; further unique obstructions/sectors, drone, blueprint progression and new powered/ranged tools remain planned. Exact proof and the distinction between development and the last package are in CURRENT_SLICE.

**September 11 Survey Recovery candidate:** cargo fragments lead from the Forest Edge arrival trail to a torn manufactured survey cabin, distinct from alien ruins. Its one-time supply chest contains a Field-pack cartridge; a full pack leaves it recoverable. Fitting the cartridge with6fiber/2wood at a placed Salvage bench expands16slots to20 without rearranging existing stacks. Existing20/24slot packs are not charged or reduced. The cartridge itself retains fitting guidance when inspected. This is the first physical discovery→equipment-capacity loop; a general blueprint collection, further pack tiers, helper drone and larger wreck chain remain planned. Exact economy and presentation remain provisional; current proof belongs in CURRENT_SLICE.

**September 11 evening Camp candidate:** emergency armor encloses the starter apron, with an open north approach and a southern personnel gap. A material-only gravel bay beside the pod fits a floor, Salvage bench and crate. Three amber-marked root/growth/rubble bundles are cleared with ordinary Omni-tool hits. The nearby amber console displays6wood/4stone/2fiber before extending the perimeter around the adjoining yard. Clearing and expansion persist; previously paid or built southern Camps retain their construction and physical storage. This is one defended expansion, not the old three-square tier UI. Exact costs/composition remain provisional; helper drone and unique natural passages are subsequent planned work. Survey/cartridge implementation is described above.

**September 11 evening phone pass (working candidate):** square backpack cells and an above-finger drag preview, rounded frames, a browser fullscreen control, closer default landscape framing and bounded right-side pinch/wheel zoom. The real movement stick supplies the resting cue. Creature observation now earns two persistent Journal notes per species by watching quietly with a clear view without detection; timing is species-specific and basic taming instructions remain available without grinding. Moving world labels retain a valid side and follow each frame. **Owner-locked harvesting behavior:** continue hitting a node until depleted whether previous drops were picked up or not. The physical-inventory integration's next-hit pickup gate is superseded. Independent and aggregate closure status lives in CURRENT_SLICE, not this design description.
...
**September 11 new owner direction (broader implementation pending):** the player's landing pod crashed after a colony/research ship or station incident. A Camp robot/drone, scattered supplies/crates/beacons/teleportation equipment and progressively larger wreck sites will tie exploration to parts, blueprints, new equipment and crafting. Draggable/sortable finite inventory, physical storage and the first16→20-slot cartridge fitting are integrated; further pack tiers, new narrative content and general blueprint progression remain planned in `CRASHLAND_PROGRESSION_PROPOSAL.md`. Exact plot, capacities, blueprint costs and recovery rules remain provisional.
