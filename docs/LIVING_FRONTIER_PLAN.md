...
| F4 Individual Wildkin | Two Mosslings can look different and remain separate companions | Shared expression across wild/follower/portrait; independent materials; capture/secure/reload; no species deduplication loss |
...
F1 starts now. The deterministic genome helper may be built independently as preparation for F4, but a helper alone is not a playable creature feature. Small complete integrations take priority over stockpiling disconnected modules. Reorder adjacent slices when dependencies or player feedback justify it; record the reason in CURRENT_SLICE.
