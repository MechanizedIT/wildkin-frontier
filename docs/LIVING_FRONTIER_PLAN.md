...
- Latest September12 steering after waking: mobile/casual first, portrait as primary layout, with landscape/desktop support. See MOBILE_IDENTITY.md for Honeycomb research and the completed M1 portrait controls decision. F7B earned research and optional body-tone guidance are now functional beside F2D selected scenery. F1B rolling terrain and F4B clearer body tones are now selected. F2E shipped-species encounter reuse and F2F cheaper upright grass are selected. Next: unify the saved world descriptor, then inspect and improve connected regional generation. Bound rendering, streaming, simulation and storage; do not claim physical-phone proof from an emulated viewport.
