...
- Latest September12 steering after waking: mobile/casual first, portrait as primary layout, with landscape/desktop support. See MOBILE_IDENTITY.md for Honeycomb research and the completed M1 portrait controls decision. F7B earned research and optional body-tone guidance are functional beside F2D selected scenery. F1B rolling terrain, F4B clearer body tones, F2E encounter reuse and F2F cheaper grass are selected. The shared world descriptor is now integrated through current generation layers while saves remain fixed-world. The bounded frontier inspector now measures the current fields; next is connected regional generation starting with dramatic plateaus. Bound rendering, streaming, simulation and storage; do not claim physical-phone proof from an emulated viewport.
- Subsequent explicit geography direction: large biomes, extreme alien heights and distinct life above/below, coherent rivers and some persistent clearing through dense areas, without a predictable global path web. [`WORLD_GENERATION_PLAN.md`](WORLD_GENERATION_PLAN.md) owns the provisional representation/generation sequence; [`REGIONAL_DIVERSITY_PLAN.md`](REGIONAL_DIVERSITY_PLAN.md) proposes six regional families and progression grammar, none of which is runtime content yet. Component modularity for effects/crops/machines is a stated architectural interest. `SIMULATION_PLAN.md` documents focused composition and the implemented crop/young process-clock reuse without an ECS migration.
