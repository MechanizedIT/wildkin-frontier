...
**Implemented subset, September 11:** finite 16-slot pack, 24-slot physical pod/crate storage, accessible-source crafting, drag/sort/split with tap alternatives, preserved migration overflow and active-expedition resume are now integrated. Death provisionally keeps packed items and loses carried XP/pending bonds. The proposal's later backpack upgrades, wreckage story, blueprint progression, unique barriers and defended Camp sectors remain planned; their suggested costs and recovery rules are not shipping contracts. The source audit linked below is the pre-cutover audit, not a current feature inventory.
