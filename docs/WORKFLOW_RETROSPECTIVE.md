...
### September12 — encounter integration and cheap grass
- Check inherited CSS insets and transforms together. Absolute top plus bottom can stretch an auto-height portrait panel; changing left from50% to an edge inset must clear translateX(-50%). Verify live taming/observation states, not only a quiet HUD.
- Native action observations must start before input. Waiting for an accessibility snapshot after clicking can miss an entire short Jump. For dynamic encounters, combine a bounded real-handler fixture, immediate screenshot and safe exit in one browser call; model/tool latency can let a live challenge finish unattended. Keep unattended-death evidence separate from timing/balance proof.
- Reuse current asset-role overrides before flattening recipes: generated rock placements already support admitted mineral visuals/yields/collision. Trace station/unlock gates from fresh saved state before claiming supplies solve progression.
- Preserve exact rejected geometry before later passes. Cheap triangles permit fuller cover, but blade orientation at the actual portrait angle still decides whether it reads as grass or flat debris.

### September12 — generation identity closure
- Seed propagation includes clearance/exclusion queries, fallback ground samples and subordinate grass scatter, not just the main generator. Test actual alternate-world placement positions against their exclusion filter; successful alternate terrain replay alone missed default-world exclusions in scenery.
- Match lifecycle scope to the actual runtime contract. A descriptor frozen for the page needs one constructor capture; per-frame key strings and live swap branches add cost and unsafe identity behavior when saved provenance is still fixed-world. Defer the seed selector and its full namespace transaction closure together.

### September12 — portrait hotbar and regional diagnostics
- A transformed toolbar becomes the containing block for fixed children. Keep the full-screen HUD wrapper untransformed and pointer-transparent; position the belt and Pack separately. A broad `span` hide also removes the shared icon wrapper: preserve `.item-icon` explicitly.
- Verify every sibling secondary action and active state at the smallest supported width. A generic narrow media rule can still lose to an earlier state-specific `!important` selector; live Natural/Clay bounding boxes caught an overlap that ordinary Release did not expose. A passing aesthetic round does not waive later correctness defects.
...
- ReportLab PDFs can use ASCII-safe streams that Git mistakes for text. Mark PDFs binary before staging and compare the staged blob with the exact local bytes; line-ending conversion can invalidate PDF cross-reference offsets.

### September12 — cap ecology and physical movement proof
- A level home center and eight outer samples can hide an interior lip. Wildlife needs adjacent-step checks across its whole movement area and a real return/bond replay. A pure AI simulation without the actual terrain/controller does not prove a cap encounter traversable.
- Preserve scoped controls: cap-only movement bounds and soft-flower clearance may differ from ordinary wildlife without changing every source. Resizing a harvestable also changes its footprint and practical strike approach; prove the collected yield, not just depletion.
- Follow supported route bends during native checks. A scripted shortcut into an older terrace is not a new terrain defect. Keep failed attempts in scratch evidence, then record the corrected ordinary walk with health and cargo.
- A hidden interaction can be a framing, range, or clear-ground decision. Inspect the exact live decision before changing a cache; a synthetic state the runtime never creates is not a useful regression. Camera-relative test movement must use the current camera basis.
- Do not print complete minified world JSON or asset objects while searching. Exclude generated data from line searches, then parse only the requested keys. Keep runtime counts distinct from source candidates and emulation distinct from real-phone performance.

### September12 — seeded provinces and portrait perception
- Region-scale height/color variety can pass deterministic and traversal checks yet remain unreadable in a close portrait camera. Judge the ordinary play view; a measured13m rise is not evidence that the player sees a ridge. After three held passes, preserve the strongest version and carry the structural camera/material/landmark gap forward honestly.
- Confirm world direction from the camera basis before describing a nearby feature as ahead: yaw0 here faces negative Z. Keep baseline/target/witness framing reproducible, and distinguish overhead diagnostics from actual play.
...
## September13 — bounded preparation and measured reuse

- Prepare detached terrain near an anticipated boundary before publishing a complete neighborhood. Prove both visual-construction and real collider-allocation failures preserve the old support/snapshot and dispose candidates; separately state that engine removal/enable/step failures are outside the guarantee. Check terrain, climbing rocks, scenery solids, disabled IDs and section activation through the shared physics path.
- A retained recipe cache and a build-local exact-point memo solve different repeated work. Keep recipe ownership bounded to the current expanded window; release point maps after synchronous construction even when it throws. Preserve separate height/full-sample API semantics and exact instance output before accepting a speed improvement.
- Use a quiet repeatable route and CPU profile to select a small optimization. Here analytical terrain work dominated scenery time; changing geometry batching would have addressed a smaller share. Preparing unused terrain can increase total work while lowering the largest pause, so report peak and total claims separately.
- Use ordinary Git line-ending settings for diff checks. A temporary `core.autocrlf=false` override manufactured thousands of CRLF trailing-whitespace reports; redirect routine diff-check output and inspect the exit/result instead of changing configuration to silence conversion notices.
