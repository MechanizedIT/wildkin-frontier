...
test('rolling transition leaves authored Camp samples exact', () => {
  ...
  for (const [x, z] of [[-50, -50], [0, -50], [50, -50], [-50, 0], [0, 0], [50, 0], [-50, 50], [0, 50], [50, 50]]) {
    assert.equal(sampleFrontier(x, z, { campHeight }).height, campHeight(x, z), `Camp height at ${x},${z}`);
  }
});

test('rolling transition preserves the established north terrace and approach heights', () => {
  const protectedSamples = [
    [0, -110, 5.976246171775798],
    [24, -114, 4.740649634730418],
    [24, -128, 8.728540728938814],
    [32, -133, 8.403194422750774],
    [42, -133, 7.603701842446358],
  ];
  for (const [x, z, expected] of protectedSamples) {
    assert.equal(sampleFrontier(x, z).height, expected, `protected height at ${x},${z}`);
  }
});
