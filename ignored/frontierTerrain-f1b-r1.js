...
import { ROCKY_TERRACE, sampleFrontierLandform } from '../src/world/frontierLandform.js';
...
function globalHeight(x, z, seed) {
  ...
  // A restrained lateral saddle gives the first northbound wet/dry transition
  // readable shape without reaching the protected terrace farther north.
  const transitionFade = smooth(clamp((z + 110) / 12, 0, 1));
  const rollingTransition = (
    hill(-4, -92, 18, 32, 1.45)
    + hill(19, -89, 17, 30, -.75)
  ) * transitionFade;
