...
test('rolling color cue distinguishes the R1 dry rise and wet bowl without changing their terrain state', () => {
  const dry = sampleFrontier(-4, -92);
  assert.equal(dry.height, 5.617401282209718);
  assert.deepEqual(dry.habitatBlend, { fernUpland: 0.6384264621010006, wetland: 0.3615735378989994 });
  assert.ok(dry.groundColorRGB[0] - 0.20681386751433928 >= .06, 'dry rise is visibly warmer');
  assert.ok(dry.groundColorRGB[1] - 0.40102504711914594 >= .045, 'dry rise is visibly lighter');

  const wet = sampleFrontier(19, -89);
  assert.equal(wet.height, 2.63568871824491);
  assert.deepEqual(wet.habitatBlend, { fernUpland: 0.373609187929262, wetland: 0.626390812070738 });
  assert.ok(wet.groundColorRGB[0] - 0.1881380988254598 <= -.05, 'wet bowl is visibly darker');
  assert.ok(wet.groundColorRGB[2] - 0.27279976797108135 >= .02, 'wet bowl is visibly cooler');
});
