...
import { planFragmentColliders } from './rock-fragment-collider.js';
import { planVoxelCollider, createVoxelDescriptor, voxelSurfaceMesh } from './rock-voxel-collider.js';
import { auditPreparedRockCollision } from './matter-physics.js';
await R.init();
const fixtures=collisionStudyFixtures(),methods=['fracture-fragments','native-voxel-0.25','native-voxel-0.5','four-sector','eight-sector-tight','eight-sector','occupancy-median','occupancy-gap','occupancy-voronoi'];
...
function draw(){
  ...
  const points=m==='fracture-fragments'?planFragmentColliders(fixture).map(f=>f.hull):m.startsWith('native-voxel')?[]:m==='four-sector'?planRockColliders(mesh,record.localCOM):m.startsWith('eight-sector')?planEightSectorControl(mesh,record.localCOM,m==='eight-sector-tight'?.1:.35):planOccupancyColliders(mesh,fixture.sample,m);
  const voxelPlan=m.startsWith('native-voxel')?planVoxelCollider(fixture.sample,m==='native-voxel-0.25'?.25:.5):null;
  const colliders=voxelPlan?[world.createCollider(createVoxelDescriptor(R,voxelPlan))]:points.map(p=>world.createCollider(R.ColliderDesc.convexHull(p))),audit=auditPreparedRockCollision(R,mesh,record,colliders);
  document.getElementById('metrics').textContent=`${fixture.name} · ${m} · ${voxelPlan?`${voxelPlan.voxelCount} collision voxels`:`${points.length} hulls`} · sampled ray error ${audit.maxGap.toFixed(3)} m · ${audit.passes?'legacy rays pass (full study still HOLD)':'fails 0.5 m legacy safety limit'}`;
  for(const [index,renderer] of renderers.entries()){
    ...
    if(index!==0)colliders.forEach((c,i)=>{const surface=voxelPlan?voxelSurfaceMesh(voxelPlan):{positions:c.vertices(),indices:c.indices()};const g=geometry(surface.positions,surface.indices);resources.push(g);
      if(index===1)scene.add(new THREE.Mesh(g,new THREE.MeshStandardMaterial({color:palette[i%palette.length],flatShading:true,side:THREE.DoubleSide,roughness:.8})));
