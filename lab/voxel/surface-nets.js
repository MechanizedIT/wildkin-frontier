...
function makeVertex(cx, cy, cz, densities, materials, n, size, spacing) {
  ...
  let px=0,py=0,pz=0,count=0,best=Infinity, material=1;
  ...
  for(let i=0;i<8;i++) if(values[i] < best && values[i] < 0) { best=values[i]; material=materials[index(cx+CORNERS[i][0],cy+CORNERS[i][1],cz+CORNERS[i][2],n)]; }
  ...
  const normal=normalize(gx,gy,gz);return { position:[px*spacing,py*spacing,pz*spacing], normal, material, shade:smoothAmbientOcclusion(values,[px-cx,py-cy,pz-cz],normal) };
}
