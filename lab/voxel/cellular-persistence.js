...
export function validateCellularState(state){
  ...
  if(!state.world?.densityEdits||typeof state.world.densityEdits!=='object'||Array.isArray(state.world.densityEdits)||!validStructure(state.world.structure)||
  ...
  for(const item of state.fractureDebris){if(!item.id||debrisIds.has(item.id)||!['temporary-physics-shard','tiny-debris'].includes(item.kind)||
      ...
      !Array.isArray(item.materials)||item.materials.length!==length||!item.materials.every(v=>v===0||v===1)||!finite3(item.position)||!finite3(item.linearVelocity)||
  ...
  const ids=new Set();for(const actor of state.actors){
    ...
    if(!actor.id||ids.has(actor.id)||state.retired.includes(actor.id)||!validField(actor)||!finite3(actor.position)||!finite3(actor.linearVelocity)||!finite3(actor.angularVelocity)||!finite3(actor.localCOM)||
      ...
      actor.domain?.id!=='rock:9212026'||actor.spacing!==.5||actor.sampleFrame?.spacing!==.5||!finite3(actor.sampleFrame.offset)||!validStructure(actor.structure))throw new Error('Corrupt cellular actor');
