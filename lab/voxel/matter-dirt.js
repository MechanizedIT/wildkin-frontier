...
import { parcelBits, parcelMaterial, removeMatterComponents } from './matter-ownership.js';
...
function localSupportWindow(samples,profile,anchor,{identityForCell,canConnect}={}){
  ...
  const started=performance.now(),result=analyzeMatterConnectivity(samples,{anchor,
    ...(identityForCell?{identityForCell}:{}),...(canConnect?{canConnect}:{})}),connectivityMs=performance.now()-started;
  if(result.status!=='OK')return {...result,workCells,connectivityMs};
  return {...result,workCells,connectivityMs,window:{low,high}};
}

function totalParcelLoss(before,after,material=null){
  let count=0;for(let z=0;z<before.size[2]-1;z++)for(let y=0;y<before.size[1]-1;y++)for(let x=0;x<before.size[0]-1;x++){
    const p=[x,y,z],a=parcelBits(before,p),b=parcelBits(after,p);for(let i=0;i<8;i++)if(a[i]&&!b[i]&&(material===null||parcelMaterial(before,p,i)===material))count++;
  }return count;
}
function crumbleWeakSamples(samples,weak,profile,material=null,protectedSamples=null){
  ...
  for(const cell of weak)for(let dz=0;dz<=1;dz++)for(let dy=0;dy<=1;dy++)for(let dx=0;dx<=1;dx++){
    ...
    if(out.densities[i]>=0||(material!==null&&out.materials[i]!==material))continue;
  ...
  const applied=selected.filter(({x,y,z})=>!protectedSamples?.has(x+nx*(y+ny*z)));
  for(const {x,y,z} of applied){const i=x+nx*(y+ny*z);out.densities[i]=Math.max(.12,-out.densities[i]+.12);out.materials[i]=0;}
  return {sample:out,clearedSamples:applied.length};
}

function localWeakCells(before,after,support,hit,profile,material=null){
  if(!before)return [];
  ...
  for(let z=window.low[2];z<=window.high[2];z++)for(let y=window.low[1];y<=window.high[1];y++)for(let x=window.low[0];x<=window.high[0];x++){
    const p=[x,y,z],bits=parcelBits(after,p),oldBits=parcelBits(before,p),
      q=bits.reduce((n,live,i)=>n+(live&&(material===null||parcelMaterial(after,p,i)===material)?1:0),0),
      old=oldBits.reduce((n,live,i)=>n+(live&&(material===null||parcelMaterial(before,p,i)===material)?1:0),0);
...
export function resolveDirtSupport(samples,hit,profile,{anchor=()=>false,previous=null,material=null,
  identityForCell,canConnect,protectedSamples=null}={}){
  const started=performance.now(),supportOptions={identityForCell,canConnect},support=localSupportWindow(samples,profile,anchor,supportOptions);
  if(support.status!=='OK')return {...support,elapsedMs:performance.now()-started};
  const crumble=support.components.filter(component=>!component.anchored&&
    (material===null||component.fragments.every(fragment=>fragment.id===material))&&
    classifyDirtFragment(component.occupiedProbes,profile)==='crumbled-soil');
  ...
  if(crumble.length){
    const result=removeMatterComponents(current,crumble,support.components,{material,protectedSamples});
    current=result.sample;clearedSamples+=result.clearedSamples;workCells+=result.workCells;
    const converted=totalParcelLoss(samples,current,material);workCells+=samples.size.slice(0,3).reduce((a,b)=>a*(b-1),1);
    crumbledProbeUnits+=converted;if(converted)crumbleComponents.push(converted);
  }
  let latest=crumble.length?localSupportWindow(current,profile,anchor,supportOptions):support;
  ...
  const weak=localWeakCells(previous,current,latest,hit,profile,material);
  ...
  if(weak.length){
    ...
    if(weakUnits<=profile.maxCrumbledProbeUnitsPerEdit){
      const result=crumbleWeakSamples(current,selected,profile,material,protectedSamples),converted=totalParcelLoss(current,result.sample,material);
      ...
      latest=localSupportWindow(current,profile,anchor,supportOptions);
